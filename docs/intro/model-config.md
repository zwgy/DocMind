...
对话模型的“上下文长度”填写**推理服务实际部署上限**，单位为 Token。例如，部署实例限制为 32K 时填写 `32768`。从远端模型列表添加时，如果页面“上下文”显示为 `-`，说明 `/models` 没有提供可用值，需要在模型配置中手动填写。Embedding、Rerank 不参与 Agent 对话上下文压缩，无需也不会保存此项。
...
对话模型还必须配置“最大输出 Token”：这是一次生成（包含服务商计入 completion 的隐藏推理 Token）可占用的最大额度。系统会从完整窗口中先扣除该预留和安全缓冲，再计算最终请求可使用的输入预算；这三个值缺失或相互冲突时，系统会在调用前明确报出配置错误，而不是按模型名称或窗口比例猜测。该值同时会作为实际模型请求的上限；调用方显式指定更小的上限可以进一步收紧，但不能突破模型配置的上限。
