...
“生成前思考”是对话模型的可选运行参数。默认跟随模型服务；当本地思考模型在回答前长时间没有文字或工具调用时，可选择“关闭（优先响应速度）”。系统会把它作为模型级 `reasoning_effort` 参数发送，因此只有明确支持该参数的 OpenAI 兼容服务才应配置；不同模型可以分别设置，不需要共享供应商级配置。关闭生成前思考会降低复杂推理深度，应结合实际问答正确性验收，而不是只比较首字符速度。
