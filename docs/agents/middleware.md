...
| 字段 | 说明 |
| --- | --- |
| `summary_prompt` | 定义摘要提取内容和输出结构；框架另行追加累计合并、输入隔离、精确值保护和输出预算协议 |

模型缓存中的完整窗口、最低输出预留和安全缓冲会写入 LangChain model profile。完整窗口按“已保存的手动/模型服务值 → LangChain 模型资料 → 系统 `default_context_window`（默认 32768）”解析，因此历史模型缺少显式配置时仍可运行；默认值不代表推理实例的真实能力，管理页会显示来源并提示核对。每次调用均对最终系统提示词、消息和工具定义做 Unicode/JSON 感知的本地保守估算，以“完整窗口 − 输出预留 − 安全缓冲”得到可用输入预算；超预算时才压缩最早的完整交互段。最低输出预留只决定压缩阈值，不会限制模型生成。
...
L3 处理当前单条用户请求内的早期闭合 API round，仍只投影安全工具载荷；最近两个闭合 round、错误和人工确认保持完整。若 L1-L3 后仍超预算，L5 以完整 API round 为单位生成私有 checkpoint。已完成的错误或已回答的确认轮次可归档，不永久占用窗口；未完成工具调用仍由协议校验保护。语义摘要路径默认保留最新 HumanMessage 和近期两个完整轮次，主模型成功后才提交状态；恢复路径按实际预算选择近期完整轮次，不强求固定数量。

摘要请求不接收 Agent 工具。默认摘要提示词使用九维任务/事实/文件/错误/待办/当前工作标签；管理员配置的 `summary_prompt` 可以完整定义另一种输出结构，框架不再强制追加九维标签，但会追加与字段结构无关的累计合并、输入隔离、精确值保护和输出预算协议。所有消息、上一版 checkpoint 和候选摘要都按历史数据处理，不能反向改变摘要任务或输出结构。

累计协议要求先继承仍有效的旧约束、决策、精确标识和待办，再吸收新事实。长期要求只有在用户明确取消、替换或更正时才失效；失效项仍保留精确标识和变更状态，不能重新写成有效要求。待办只有在存在可靠工具结果、交付物、其他可核验证据，或用户明确确认时才转入进展；助手仅声称“已记录、已确认、已回复”不代表完成。已经完成的旧轮临时输出格式和工具使用要求只保留结果，不得继续作为待办、当前工作或下一步。恢复时以最新真实用户消息确定本轮任务和回答形式，同时遵守未被明确变更的长期约束；checkpoint 中的 `current work` 和 `next step` 仅在与最新消息一致时继续。

精确锚点门禁只从本轮被压缩的 HumanMessage 和分块时由代码注入的原始用户锚点提取路径、标识符、版本号及错误码。生成的上一版 checkpoint 不再作为强制精确值来源，避免旧摘要中的错误或省略路径逐代晋升为必须保留的事实。一个 Human turn 包含大量工具调用、需要滚动分块摘要时，每个分块只在摘要模型输入内重复该段原始 HumanMessage，锚点不写入 checkpoint 或主模型消息，也不复制图片和工具正文。

摘要输出上限按部署窗口动态计算：取部署最低输出预留、窗口八分之一与 20K 上限形成摘要 cap，因此 32K、64K、128K、256K 分别可自然扩展到约 4K、8K、16K、20K，而不依赖模型名称。摘要输入再从完整窗口中扣除该输出 cap 和安全缓冲。若 provider 仍明确返回 prompt too long，只移除最旧 20% 的完整 API round、收紧输入估算并重试一次；原消息已经归档，重试不会删除不可恢复原文。

摘要增强整次最多等待 30 秒，分块、锚点修复与唯一一次 PTL 重试合计最多调用模型 4 次，不再在多个归档边界反复生成摘要。同步摘要入口也使用原生异步调用和取消边界；事件循环中的调用方使用 `awrap_model_call`。供应商实测输出用于检查生成额度，完整主请求估算单独负责准入，不混用两种预算。

摘要超时、空响应、`finish_reason=length`、超限、锚点校验或普通生成失败不再中断会话。恢复路径先把原始请求消息及标记为非权威的旧摘要归档，再按预算保留最新用户请求和最近的完整 API round，使用准确归档入口继续调用主模型，提交质量为 `archived`。若仍收到供应商溢出，恢复至少移除最旧 20% 完整轮次，避免按旧估算装回同一请求。异步整个准备阶段最多 40 秒，恢复归档最多再等待 10 秒；归档不可用时仅缩小本次模型视图，保留完整持久消息，不推进水位、不生成虚假路径。当前用户输入本身放不下时使用归档读取入口；若同时无法归档，则让模型明确提示用户分段发送，原始输入仍留在检查点。固定系统/工具配置无法容纳最小请求、已损坏的工具协议、用户取消和主模型自身故障不被伪装成压缩成功。

默认九维策略标签不齐只记录 `format_unverified`，自定义策略记录为 `custom`，均不触发格式修复调用。需要旧细节或逐字核对时用 `ls/read_file` 回查 `/outputs/conversation_history/`；语义摘要不能保证本地小模型无损记忆。`SkillsMiddleware` 维护的激活状态和工具/MCP 绑定，以及 Todo、附件、artifact 等独立 state，不会被 `Overwrite(messages)` 覆盖。

只有请求超过准入预算或进入 provider overflow 恢复时，压缩器才会发布 `context_compaction` 事件。L1、L2、L3 各发布 `finished/skipped`，L5 发布 `started` 后以 `finished/failed` 记录摘要阶段结果；摘要失败或无可用摘要边界时，确定性恢复另行发布 `recovered`，通过 `recovery_mode=archived/request_only` 区分是否提交归档收缩。若前三级已经满足预算，L5 发布 `skipped`。常规各级周期通过 `cycle_id` 关联，按 `sequence=1/2/3/5` 排序。
