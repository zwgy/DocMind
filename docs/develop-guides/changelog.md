...
- 完善未入库附件的嵌入式同步闭环：用户第一次打开助手后，chat-iframe 查询附件状态并在发现 `pending_sync` 时通过通用消息协议通知父页面 SDK；仅浏览宿主页面不会触发附件下载或上传。SDK 使用接入方提供的 `source_url` 下载附件并上传现有 DocMind 入库接口，同时回传下载、上传、完成或失败状态。聊天区顶部在可选通知轮播下方居中展示准备进度，解析未完成时保留用户输入并提示等待，解析就绪后自动刷新摘要并提示可提问；上传响应不确定时先以后端查询结果对账，避免重复上传。首次打开后最小化或关闭悬浮窗不会中断状态；宿主 SPA 以 `source_system + source_function_id + business_id` 识别页面，通过 `setPageContext()` 切页时中止并清空旧 SDK 任务、重载 iframe 清空旧状态。页面内来文 ID 在 `document_metadata.source_doc_id` 中声明一次，同一来文下的附件使用各自的 `source_file_id`；SDK 将来文 ID 转换为后端查询和上传所需的身份字段。未传 `source_doc_id` 时 SDK 仅为兼容旧的一页一来文接入而使用 `business_id` 兜底，二者语义不同。同一批附件复用进行中的任务，失败后允许重试，旧 SDK 与非嵌入场景继续使用原有直传路径。本次不增加特定系统适配器、自定义 `fileLoader` 或新的后端接口。
