...
- 补齐来文确认的单一事务边界：Repository 为调用方 `AsyncSession` 提供无提交的锁定读取与更新方法，确认用例不再在事务外预检；审计写入异常时，来文确认状态、批次冻结、候选状态、任务和审计记录会一并回滚。新增内置 `scheduled-task` Skill 和创建、查询、暂停/恢复、取消四个原生个人任务工具，创建幂等键由认证身份、会话线程和框架注入的工具调用 ID 生成；Skill 自动提供给顶层已认证 Agent，子 Agent 不注入且工具在读取 Skill 前对模型保持隐藏。
