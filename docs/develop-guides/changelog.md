...
- 新增 PostgreSQL 驱动的文档级来文接入任务与专用串行 Worker：历史任务仅在夜间启动，即时来文优先，Redis 仅传递带令牌的短消息，文件与解析产物仍由 MinIO 保存。
- 来文管理重构为“来文”和“处理任务”页签：处理任务支持查看中文业务进度、失败摘要和完整诊断，对失败/取消任务重试、对未完成历史任务提升优先级；下载地址失效时支持管理员刷新来源清单后重新入队。历史登记保留单一 `POST /api/incoming-documents/ingest-batches` 路径，一次支持 1 至 200 份来文并按 `source_system + source_document_id` 幂等复用，不再要求批次键、批次名称或提交步骤；配套脚本按数量和 5 MiB 请求上限分块，支持网络失败后原样重跑并报告输入冲突。
- 完善历史来文上传脚本的内置使用说明：`--help` 现在包含完整参数与环境变量含义、单条/批量 JSON 示例、字段约束、幂等规则和逐项返回状态，脚本可独立交付给接入方使用。
- 文档解析 OCR 改为系统全局默认配置：来文任务首次执行时固化引擎与高级参数快照，知识库上传选择“使用系统默认配置”时继承同一值，显式单次选择不混入全局其他引擎参数。高级参数可透传 MinerU 后续请求字段，固定 ZIP 响应协议字段仍不允许覆盖。
- 来文批量入库复用已生成的受内部服务控制的 Markdown：知识库仍保存原件记录并执行分块、向量化和索引，但不再重新调用文档解析器或 OCR，避免重复消耗 MinerU 与本地模型资源。
- 修复来文专用 Worker 未监听 `arq:incoming`、未挂载持久化配置目录且未同步运行时配置的问题；现可消费专用队列并使用管理端配置的模型和解析参数。成功或取消任务同步更新接入阶段，避免管理页长期显示“已登记”。
- 来文历史调度窗口和并发上限改为运行时配置：默认仍为上海时间 `18:00` 至次日 `07:30`、并发 1；Dispatcher 与 Worker 每五秒同步新值，ARQ 进程保留最多 4 个安全执行槽位，实际准入由在线配置控制，降低并发不会中断在途任务。
- 处理任务页新增无需重启的调度设置，可在线调整历史窗口、来文并发、来源下载超时和自动重试次数。下载超时、网络异常和 HTTP 5xx 可按配置有限重试；HTTP 4xx、HTML 登录页和非法文件直接保留失败原因，供管理员修正来源后人工重试。
- 修复运行时配置批量校验时意外重新加载 `base.toml`、导致管理员提交的新值被旧磁盘配置覆盖的问题；调度设置及其他可修改配置现在先在无副作用候选值上完成字段和跨字段校验，再一次性生效。配置元数据中的默认工厂值会先解析为可序列化结果，避免管理端保存已生效却收到 HTTP 500。
- 修复本地模型在任务条目中以 `null` 或字段名占位词 `scope` 表示未明确接收人时导致整份来文抽取失败的问题；提示词明确 `recipient_scope` 只能填写 `named`、`all` 或 `unknown`，无业务语义的占位值按 `unknown` 处理，其他非法范围及人员组合仍保持严格校验。
...
- 完善未入库附件的嵌入式同步闭环：用户第一次打开助手后，chat-iframe 查询附件状态并在发现 `pending_sync` 时把接入方提供的 `source_url` 以 JSON 提交现有 DocMind 入库接口，由后端直接下载附件，和 multipart 已上传文件统一交给 `ingest_files()` 保存并投递解析，浏览器不再先下载再上传，也不再受附件服务 CORS 限制。下载支持 HTTP/HTTPS、重定向、60 秒超时和 100 MB 上限，拒绝非文档 HTML 响应并保留明确失败原因；不固定附件 host 或路径，不增加特定系统适配器、自定义 `fileLoader`、独立接口或数据表。聊天区顶部在可选通知轮播下方居中展示下载、解析和完成状态，解析未完成时保留用户输入并提示等待，解析就绪后自动刷新摘要；首次打开后最小化或关闭悬浮窗不会中断已提交到后端的任务。同一批附件在首次展开、鉴权完成和页面恢复可见等事件并发触发时复用进行中的状态查询，只有解析尚未终结时才按 2 秒间隔继续轮询，进入 `ready/failed` 后停止。宿主 SPA 以 `source_system + source_function_id + business_id` 识别页面并隔离会话；来文按 `source_system + source_doc_id` 复用解析结果，附件再由 `source_file_id` 区分。JSON 与 multipart 入库均在 `document_metadata.source_doc_id` 声明来文 ID，上传接口不再接收只属于页面会话的 `source_function_id/business_id`；父 SDK 仅在该字段缺失时使用页面 `business_id` 兼容一页一来文接入。现有 multipart 直传方式继续兼容；父 SDK 中原浏览器下载、上传和附件状态消息协议已删除，避免保留两套入库链路。
...
- 闭合定时 Agent 结果的发现与续聊链路：运行记录新增公开 `conversation_thread_id` 和最终回复摘要、消息 ID、去重产物数投影，Worker 终态同步失败时由 Dispatcher 每 10 秒有界对账恢复，历史终态只回填摘要而不制造未读。Web 与 chat-iframe 的原会话列表通过一次请求平铺当前普通会话和当前用户跨 Agent/scope 的定时会话，以 `MessageSquare / Clock3` 区分；chat-iframe 保持服务端会话顺序不变，重新打开列表时仅滚动定位当前会话。定时会话允许重命名、置顶；个人定时结果会话可独立删除，来文历史 Agent 会话继续保护。任务收件箱和运行历史展示状态、完成时间、摘要、产物数及“查看结果”，按 ID 定位分页外会话，消息成功加载后只标记对应 run 已读。已有会话续聊使用 Conversation 绑定 Agent，绝不回退到宿主或当前默认 Agent。
- 增加定时任务与收件箱历史清理闭环：个人通知和 Agent 任务在没有排队/执行中运行时可由所有者确认后物理删除任务定义、运行、收件事件和任务审计，结果 Conversation、消息和产物独立保留；来文任务固定为通知且不再归上传者所有，上传继续允许普通认证用户，启用后仅管理员/超级管理员可暂停、恢复或取消，终态“删除”只对当前管理员隐藏。收件箱支持单条删除和按分类删除已读，个人来源条目物理删除，来文来源条目仅对当前接收人隐藏，其他用户及未来周期事件不受影响。Web 与小助手统一使用“取消”表示停止未来触发、“删除”表示清理当前列表。
- 精简个人通知历史的重复信息：Web 与小助手的“我的定时 → 历史”卡片不再展示已由“收件箱 → 通知”承载的通知正文，只保留任务名称、状态、调度规则和最近触发时间；任务详情仍可查看完整正文，Agent 历史继续展示执行指令，来文管理历史保持原展示。
- 补充定时 Agent 的来源会话导航：今后从 Web 或小助手会话内创建的个人任务会固化创建会话 ID，收件箱“任务”统一以同尺寸操作栏展示“查看结果 / 查看来源 / 删除”；“查看来源”只切回创建任务的会话且不改变运行未读状态，HTTP 创建、历史无快照任务或来源会话已删除时不建立隐式关联。双端将“全部已读 / 删除已读”相邻展示，前者使用 `CheckCheck` 和主色，后者使用 `Trash2` 和柔和错误色，窄宽度时整组换行。
...
- 优化 chat-iframe 的个人定时任务体验：邮箱图标调整为时钟入口，单一“定时中心”默认展示当前认证用户的“我的定时”，支持按进行中、已暂停和历史查看通知与 Agent 定义，并直接编辑、暂停、恢复或取消；收件箱改为仅展示已发生事件，通知页包含已投递和投递异常，任务页只聚合 `agent` 动作的执行状态。时钟入口、收件箱及通知/任务页签显示高对比度未读数，不把待触发任务计为未读；页面可见时 30 秒轮询，收到事件后在标题下显示跨会话的用户级轮播，轮播只展示正文、长内容连续水平循环，点击即已读并打开对应分类。一级页签保持紧凑左对齐，两组二级页签统一布局和选中效果；“全部已读”仅在当前分类存在未读事件时显示，空列表按当前页签提供图形和准确文案。小助手与 Web 的任务列表同步补充通知正文或 Agent 指令、动作摘要、可读调度规则及下一次/最近触发时间；常见 Cron 转换为“每周五 17:00”“工作日 09:00”等描述，复杂表达式保留原值。时区继续参与调度计算但不进入列表、卡片或详情；个人任务编辑使用中文 24 小时制日期时间选择器并保持任务时区下的本地墙钟值，取消任务使用居中警示弹窗和等宽操作按钮。收件箱条目与“我的定时”统一使用可见边框；编辑保存成功后立即关闭弹窗并给出成功反馈，保存失败在弹窗内显示真实原因，后续列表刷新失败不会再遮蔽已有数据或误报保存失败。个人定时卡片将“单次提醒 / 重复提醒”或 Agent 对应的“单次执行 / 重复执行”拆为独立高对比标签，调度规则紧邻加粗展示；通知任务不再重复展示通知标题，Agent 任务仍保留目标 Agent。定时与收件箱正文统一提升为 14px、中等字重和两行展示，在保持原有排布的前提下强化任务内容。修正编辑边界后，尚未生成运行的一次性任务以及处于进行中或已暂停状态的周期任务均可编辑；周期任务即使已有历史运行也只修改未来规则，暂停编辑后仍保持暂停。多条未读不再按 4.5 秒纵向切换，而是以通知/任务图标和分隔点组成最多 5 条的单一连续轨道；滚动时长按内容宽度计算，新轮询批次只在完整循环结束后替换，编号和右侧数量均不重复展示。单条未读即使正文未溢出也会保持连续轮播，并以至少一个视窗宽度隔开循环副本；轮播背景取消左右闭合边框，滚动文字在两侧自然渐隐。
...
- 来文管理新增归档与删除保护：默认列表排除归档项并支持 `include_archived` 显式查询，详情返回候选数、已启用任务数和删除/归档能力字段；已确认、已启用或已有候选审计引用的来文拒绝物理删除并返回稳定 `incoming_has_audit_reference`，草稿删除按候选、批次、抽取记录和来文的外键依赖逆序执行。删除确认对超过 6 位的来源单号校验后 6 位，对不超过 6 位的来源单号校验完整值，避免短单号无法确认删除。
...
- 修复本地模型摘要失败导致会话中断：摘要增强受整次 30 秒及 4 次调用额度约束，超时、空响应、截断、超限或校验失败改为确定性归档恢复并继续主模型；归档不可用时仅缩小本次请求且保留完整持久历史。旧摘要中的生成路径不再作为下一代强制精确事实，生成额度与请求准入分开校验；L5 允许归档已完成错误和已回答确认轮次，避免长期挤占窗口。新增同步/异步失败、超时取消、归档不可用、同一 LangGraph 检查点连续追问的回归覆盖。此项取代此前“摘要失败不调用主模型”的终止策略。
...
- 调整 Agent 上下文准入校准：稳定校准键不再包含动态工具 schema；冷启动及尚无同规模样本的请求保留字符保守估算，已有可靠 provider usage 后按基础估算规模桶保存并应用最大正误差，避免中文/JSON 字符兜底长期接近真实输入的两倍而反复误触发压缩。schema 与 system hash 仅用于诊断，避免多 Skill/MCP 工具变化时丢失已验证的预算保护。
...
- 补齐无页面来文时的全局来文问答入口：欢迎区使用紧凑的图标标题、能力摘要和响应式双列问题入口，并根据当前页面、当前来文或无上下文三种状态提供对应示例；示例只回填和聚焦输入框、不自动发送。来文业务工具同步增加标题、文号、发文单位独立筛选，通用关键词覆盖标题、文号、发文单位、已有摘要和附件名称；选定来文后可继续读取摘要、结构化关键信息和附件 Markdown 原文，也可把指定原始主文件或附件安全写入当前会话产物目录供预览或下载，不向模型和用户暴露 MinIO 地址或宿主机路径。业务检索结果按来文日期倒序稳定返回，准确区分主文件是否存在和主文件之外的附件数量；列表检索命中多份时直接分页展示，来文 Skill 明确给出 `date_from`、`date_to`、`page` 和 `page_size` 参数示例并说明无需额外排序参数，避免本地小模型漏传日期范围后反复查询；搜索结果只返回候选识别与列表展示所需字段，完整摘要和分类证据继续由详情工具提供，降低长会话中列表结果被外置后反复读取的概率；单篇查看、解读或下载的目标不唯一时由读取工具边界强制要求用户选择，并在恢复运行时识别已提交的结构化选择；同批并行读取只产生一次中断，避免本地模型自行选择第一份、重复追问或制造无法恢复的多中断。原文核验先以字面关键词定位，再读取不超过 50 行的小窗口，避免大文件盲读触发上下文压缩后把摘要误称为原文；原文读取和原件下载工具兼容本地模型将文件 ID 数组编码为 JSON 字符串的调用方式，其他非法输入仍由严格 Schema 拒绝。
- 原文模式省略文件 ID 时默认读取主文件，使按文号核验主文无需先加载整套结构化结果；指定附件仍必须使用附件清单中的真实文件 ID。
- 单篇少量条款核验由来文领域工具按字面短语确定最近的条、款标题并返回不超过 50 行的原文窗口，避免把条款解析交给模型或子智能体后发生引用位置漂移。
- 来文与知识库 Skill 改为分别正向描述自身数据范围和能力，不在一个 Skill 中点名、排斥或指挥另一个 Skill；模型偶发路由偏航优先通过自包含提示和自然语言纠正处理。
- 删除通用沙箱对来文输出目录的读取行数和 shell 命令特判；文件读取继续由通用 Token 预算统一限制，条款定位和有限结果由来文模块自身负责。
- 来文原文核验保持原子工具组合：`read_incoming_document` 只负责把指定主文件或附件 Markdown 写入当前会话，Agent 再用通用 `grep` 和不超过 50 行的 `read_file` 窗口定位、核验条款。移除同时承担文件选择、下载、文本检索和条款解析的 `locate_incoming_document_text`，复杂任务继续由通用 Todo/ReAct 编排；精确文号且只核验主文件时跳过完整结构化详情，避免无关大结果进入上下文。
- chat-iframe 改为按接入方显式启用的上下文展示“问网页”“问文件”：未调用 `setPageContent()` 或 `setFiles()` 时分别隐藏对应入口，两者均未调用时隐藏整行上下文操作且不显示来文解析摘要；`setPageContent()` 无参调用可手动抓取当前整页，传参时使用指定内容，初始化不再自动抓取，普通对话不会携带空页面上下文。
- 修复 chat-iframe 生产入口在部分嵌入式浏览器中由 Vite `modulepreload` 兼容脚本触发 `MutationObserver.observe` 控制台异常的问题；目标环境均为原生支持 `modulepreload` 的现代浏览器，构建改为保留原生预加载并关闭不必要的兼容脚本。
...
- chat-iframe 在运行中仍可从累积 `agent_state.artifacts` 推断新增文件；收到服务端 `finished.presented_artifacts` 后改以该本轮权威列表替换临时结果，避免中断后恢复或历史文件缺失时把旧产物重复挂到新回复。
- Office 导出 Skill 将对应格式 reference 设为写入定义前的强制步骤；首次导出失败时不再读取、搜索、执行命令或原地编辑旧 JSON，而是按错误完整重建 `-retry.json` 并仅重试一次，减少本地小模型在定义纠错中的无效循环。
...
- 修复少量长分类数据在 ECharts 自动抽样下遗漏横轴标签的问题；四项以内的类目轴固定展示全部标签，避免双项对比图只标出其中一项。
- 流程图改为在 `render_flowchart` 调用中直接传入节点和连线，移除容易被本地模型写坏的中间 JSON 文件，并禁止为代写流程定义而调用子智能体；默认只绘制用户明确要求的 2 至 10 个节点，不扩展整篇材料；外部节点 ID 改为渲染前映射的任意短文本，判断节点支持两个及以上分支，渲染器一次返回全部拓扑错误，同时保留连通性与 SVG 安全校验。工具 Schema 明确边只能原样引用节点 ID、`output_name` 只能位于顶层；无效边会同时返回具体引用和有效 ID，Pydantic 参数错误返回字段与中文原因；失败后从最小完整模板重建且最多重试两次，避免本地模型逐项修补造成无效循环。
- 流程图工具的公开 Schema 继续要求 `definition` 为对象；模型校验边界同时兼容本地小模型偶发生成的合法 JSON 对象文本，其他字符串仍按原有严格校验拒绝，避免参数格式重复失败而不放宽流程拓扑约束。
...
- 完善预算驱动的上下文恢复：被裁剪的完整交互段会先写入线程隔离的不可变 JSONL 归档清单，并把最新路径随私有摘要原子提交；摘要模型返回完整但略超目标的候选时，复用既有修复调用执行至多一次有界压短并重新校验，仍超限、输出截断、丢失精确事实或暂时不可用时明确失败并保留旧 checkpoint，不再用字符截断或归档回执伪造 `degraded` 摘要。候选输出同时按 provider 实测用量和最终主请求的本地保守估算校验，避免两种 token 口径不一致时直到最终装配才失败；首个完整 API round 留出的空间仍不足时，L5 会继续扩大归档边界，而不是直接终止整轮任务。模型重试不再吞掉 `ContextOverflowError` 或把它伪装为助手回复，而是交由摘要恢复后仅重试一次。
...
- 通用 Agent 的 Todo 提示只按任务复杂度判断：多个步骤相互依赖且需要跟踪进度或根据中间结果调整后续操作时才制定计划；单步问答、单次查询和已有内容的直接导出不创建待办，独立子任务也不得代替主任务的进度管理。该约束不绑定业务类型或交付物格式。
...
- Agent Run 创建与评估接口在 Pydantic 边界统一限制 `meta.request_id` 和 `resume_request_id` 不超过数据库支持的 64 个字符，超限请求返回 422，不再在写入运行记录时泄漏为数据库 500。
