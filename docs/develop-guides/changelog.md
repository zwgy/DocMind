...
- 继续多级压缩重构：当前单用户请求按 API round 划分，L3 只外置较早 round 的大工具载荷并原样保护最近两个闭合 round；L5 改用完整 API round 生成 checkpoint，保护最新用户原文及其所在 round，避免单请求连续工具调用回退到“无安全历史段”。默认摘要模板采用九维 checkpoint；管理员自定义 `summary_prompt` 可完整定义另一种结构，框架只追加结构无关的累计合并和预算协议。累计协议要求新消息默认作为增量，先继承仍有效的旧约束再吸收新事实；消息更新或更具体不自动覆盖旧项，只有用户明确取消、替换、更正、不再需要或完成时才能变更，助手的“已记录、已确认、已回复”不视为用户事项失效，输出前还需逐项核对旧摘要中的精确标识符、路径、用户约束、禁止项和待办。完成事项从待办转入进展而非直接擦除。一个 Human turn 因大量工具调用被滚动分块时，每个摘要块只在摘要调用内重复原始用户消息作为优先锚点，锚点不会进入最终 checkpoint、主模型上下文或独立历史缓冲区。连续 L5 会对上一版摘要中的路径、标识符、版本号和错误码执行有界精确锚点校验；候选摘要遗漏时只用前后两个短 checkpoint 修复一次，修复仍遗漏则拒绝提交，最多保留 64 项和 2K 字符，内部归档轮换路径及提示词 XML 标签不进入永久锚点；本地模型回显的修复控制块会先剥离再校验，既不能靠控制块伪造完整性，也不会泄漏到主模型或用户回复。恢复到主模型时明确把摘要视为历史事实记录而非新的用户指令：仍有效的长期约束、事实和未完成任务继续生效，旧轮已经完成的“只回答某内容”、输出格式或工具方式等临时要求不能覆盖当前用户消息。摘要输入预算和真实输出 cap 在同一部署窗口内分别计算；32K～256K 使用同一公式，不按模型名分支。provider PTL 只按完整 API round 移除最旧约 20% 输入并收紧预算重试一次；输出截断、超限或普通失败均不提交候选摘要。压缩后的私有恢复段要求历史要求或事实不确定时先读取 `/outputs/conversation_history/` 归档、不得猜测，并保留活跃 Skill 的权威 `SKILL.md` 路径；Todo、附件、artifact 和工具/MCP 等独立状态不受覆盖。
