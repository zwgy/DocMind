...
- 完善本地模型工具问答的可见输出：统一要求工具调用直接执行、回复语言跟随用户，并明确文件绝对路径和 `grep` 字面搜索契约；来文原文核验会在当前页面上下文中直接给出 `read_incoming_document → read_file` 的唯一读取链路，禁止猜测附件路径或用 `execute/glob/ls` 搜索业务附件，读取失败时也不得把摘要改写成原文；条款号和逐字引用必须由同一段工具结果共同支持。chat-iframe 会修复模型生成的 `**重点**正文` 行内强调格式，压缩状态变化也会自动滚动到可见区域。真实模型验收新增来文答案基准，关键事实错误、内部计划泄露和异常工具循环均不得判定为通过。
- 优化本地思考模型的首个可见事件延迟：模型缓存会合并供应商与单模型的 `extra.parameters`，运行时统一透传供应商支持的模型参数；Chat 模型配置新增“生成前思考”，可为支持 `reasoning_effort` 的 OpenAI 兼容服务按模型关闭或调整思考强度。中文问答同时约束工具调用前后说明必须使用中文，只读查询不得误用文件写工具；响应速度仍须与答案正确性一起验收。
