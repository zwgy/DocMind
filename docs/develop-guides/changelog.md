...
- 增加定时任务与收件箱历史清理闭环：个人通知和 Agent 任务在没有排队/执行中运行时可由所有者确认后物理删除任务定义、运行、收件事件和任务审计，结果 Conversation、消息和产物独立保留；来文任务固定为通知且不再归上传者所有，上传继续允许普通认证用户，启用后仅管理员/超级管理员可暂停、恢复或取消，终态“删除”只对当前管理员隐藏。收件箱支持单条删除和按分类清空已读，个人来源条目物理删除，来文来源条目仅对当前接收人隐藏，其他用户及未来周期事件不受影响。Web 与小助手统一使用“取消”表示停止未来触发、“删除”表示清理当前列表。
- 精简个人通知历史的重复信息：Web 与小助手的“我的定时 → 历史”卡片不再展示已由“收件箱 → 通知”承载的通知正文，只保留任务名称、状态、调度规则和最近触发时间；任务详情仍可查看完整正文，Agent 历史继续展示执行指令，来文管理历史保持原展示。
- 补充定时 Agent 的来源会话导航：今后从 Web 或小助手会话内创建的个人任务会固化创建会话 ID，收件箱“任务”统一以同尺寸操作栏展示“查看结果 / 查看来源 / 删除”；“查看来源”只切回创建任务的会话且不改变运行未读状态，HTTP 创建、历史无快照任务或来源会话已删除时不建立隐式关联。双端“清空已读”改用中性的列表清理图标和文字色，危险色仅用于单条删除。
