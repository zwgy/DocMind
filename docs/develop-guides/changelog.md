...
- 补齐无页面来文时的全局来文问答入口：欢迎区使用紧凑的图标标题、能力摘要和响应式双列问题入口，并根据当前页面、当前来文或无上下文三种状态提供对应示例；示例只回填和聚焦输入框、不自动发送。来文业务工具同步增加标题、文号、发文单位独立筛选，通用关键词覆盖标题、文号、发文单位、已有摘要和附件名称；选定来文后可继续读取摘要、结构化关键信息和附件 Markdown 原文，也可把指定原始主文件或附件安全写入当前会话产物目录供预览或下载，不向模型和用户暴露 MinIO 地址或宿主机路径。
