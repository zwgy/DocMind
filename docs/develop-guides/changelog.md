...
- 通用 Agent 的 Todo 提示明确区分单步问答与复杂任务：生成交付物前还需查询、读取、核验、分析或生成素材时，必须在首次业务操作前制定总计划，不得用子任务代替；每步完成或失败后更新状态，失败时调整后续计划，并在结果验证后结束。该约束不绑定来文、知识库、Office 或可视化模块。
