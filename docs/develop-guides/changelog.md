...
- 单篇少量条款核验由当前 Agent 直接读取原文小窗口，每次 `read_file` 硬性限制为不超过 50 行；命中款项但窗口缺少父级条款时必须向前找到最近的条款标题，只有共同核验标题和正文后才标注条款号，避免子智能体转述或模型推断造成引用位置漂移。
- 明确来文业务库与知识库 Skill 的路由边界：用户提到来文、收文、文号或来文标题时必须使用 `incoming-document`，来文无需先入知识库；仅在用户明确指定知识库或已入库资料时使用 `knowledge-base`，避免无页面来文场景错误返回“知识库未找到”。
- 来文原文核验改为由沙箱文件边界强制限制单次最多读取 50 行，并禁止 shell 直接访问系统生成的来文原文目录；Skill 同步要求每次只用一个字面短语定位，禁止把多个候选以正则形式传给字面匹配工具，避免定位失败后扫描整篇长文并误判父级条款。
...
- 流程图改为在 `render_flowchart` 调用中直接传入节点和连线，移除容易被本地模型写坏的中间 JSON 文件，并禁止为代写流程定义而调用子智能体；默认只绘制用户明确要求的 2 至 10 个节点，不扩展整篇材料；外部节点 ID 改为渲染前映射的任意短文本，判断节点支持两个及以上分支，渲染器一次返回全部拓扑错误，同时保留连通性与 SVG 安全校验。
