...
- 新增统一收件箱接口：通知按接收人事件分页，任务按所有者任务聚合展示；异常运行仅向所有者写入幂等任务状态更新，正常通知不会在两个页签重复出现。未读数量将通知按条目、任务按不同任务分别统计，所有已读操作均按当前认证用户限定并可重复调用。通知游标的未读优先级改用 SQL `CASE` 计算，避免 ORM 对 Boolean 的大小比较在第二页查询时报错。
