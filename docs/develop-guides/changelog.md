...
- 修正个人定时通知与 Agent 执行型任务的创建边界：`create_personal_scheduled_task` 不再把缺失的动作类型默认成通知，模型必须显式选择 `notification`（只到点提醒）或 `agent`（到点进入独立会话和 Agent Run 执行）。新增按当前用户权限查询可执行顶层 Agent 的 Skill 工具；用户未指定 Agent 时由创建服务在事务内固化当时的默认顶层 Agent，只有明确选择其他 Agent 时才查询并在必要时反问。
...
- 闭合定时 Agent 结果的发现与续聊链路：运行记录新增公开 `conversation_thread_id` 和最终回复摘要、消息 ID、去重产物数投影，Worker 终态同步失败时由 Dispatcher 每 10 秒有界对账恢复，历史终态只回填摘要而不制造未读。Web 与 chat-iframe 的原会话列表通过一次请求平铺当前普通会话和当前用户跨 Agent/scope 的定时会话，以 `MessageSquare / Clock3` 区分，当前选中会话保持显示层置顶；定时会话允许重命名、置顶但禁止删除。任务收件箱和运行历史展示状态、完成时间、摘要、产物数及“查看结果”，按 ID 定位分页外会话，消息成功加载后只标记对应 run 已读。已有会话续聊使用 Conversation 绑定 Agent，绝不回退到宿主或当前默认 Agent。
...
- 修复周期任务反问后的恢复创建：结构化回答以 `ToolMessage` 回放，创建工具以最新 `HumanMessage` 隔离当前请求，并只接受紧邻创建调用的 `ask_user_question` 回答映射或创建工具自身的标准反问结果，不再匹配问题 ID、文案或语言；旧轮次、已被后续工具调用跨过、非结构化和空回答均不会放宽时间校验。
- 优化 chat-iframe 的个人定时任务体验：邮箱图标调整为时钟入口，单一“定时中心”默认展示当前认证用户的“我的定时”，支持按进行中、已暂停和历史查看通知与 Agent 定义，并直接编辑、暂停、恢复或取消；收件箱改为仅展示已发生事件，通知页包含已投递和投递异常，任务页只聚合 `agent` 动作的执行状态。时钟入口、收件箱及通知/任务页签显示高对比度未读数，不把待触发任务计为未读；页面可见时 30 秒轮询，收到事件后在标题下显示跨会话的用户级轮播，轮播只展示正文、长内容连续水平循环，点击即已读并打开对应分类。一级页签保持紧凑左对齐，两组二级页签统一布局和选中效果；“全部已读”仅在当前分类存在未读事件时显示，空列表按当前页签提供图形和准确文案。小助手与 Web 的任务列表同步补充通知正文或 Agent 指令、动作摘要、可读调度规则及下一次/最近触发时间；常见 Cron 转换为“每周五 17:00”“工作日 09:00”等描述，复杂表达式保留原值。时区继续参与调度计算但不进入列表、卡片或详情；个人任务编辑使用中文 24 小时制日期时间选择器并保持任务时区下的本地墙钟值，取消任务使用居中警示弹窗和等宽操作按钮。收件箱条目与“我的定时”统一使用可见边框；编辑保存成功后立即关闭弹窗并给出成功反馈，保存失败在弹窗内显示真实原因，后续列表刷新失败不会再遮蔽已有数据或误报保存失败。个人定时卡片将“单次提醒 / 重复提醒”或 Agent 对应的“单次执行 / 重复执行”拆为独立高对比标签，调度规则紧邻加粗展示；通知任务不再重复展示通知标题，Agent 任务仍保留目标 Agent。定时与收件箱正文统一提升为 14px、中等字重和两行展示，在保持原有排布的前提下强化任务内容。修正编辑边界后，尚未生成运行的一次性任务以及处于进行中或已暂停状态的周期任务均可编辑；周期任务即使已有历史运行也只修改未来规则，暂停编辑后仍保持暂停。多条未读不再按 4.5 秒纵向切换，而是以通知/任务图标和分隔点组成最多 5 条的单一连续轨道；滚动时长按内容宽度计算，新轮询批次只在完整循环结束后替换，编号和右侧数量均不重复展示。
- Web 新增独立“定时任务”页面和管理员“待确认”页签：候选可编辑通知、`at / interval / cron` 调度、时区和人员目录范围，预览复用后端计算，校验错误会阻止启用；拒绝必须填写原因并再次确认。尚未生成运行的一次性任务，以及处于进行中或已暂停状态的周期任务支持相同的调度编辑和运行历史查看，接收人固定为任务所有者。候选人员选择复用已有、按管理员部门范围过滤的账户访问选项接口，避免新增与既有权限边界重复的人员目录 HTTP 接口。
...
- 新增来文任务候选构建、校验、编辑、启用、拒绝和来文确认批量启用：候选固定绑定本次抽取 `run_id`，在同一数据库事务中冻结批次、复验人员和创建幂等任务；字段不完整、人员无效或时间过期的候选保留结构化错误，不阻塞来文确认。
- 来文抽取 `TaskItem` 补齐通知、规范化调度、时区、人员范围及来源定位契约；解析和模型调用保持在事务外，抽取成功后才以短事务写入候选、批次与来文完成状态。候选写入失败会显式标记为可重试失败。
- 新增 `/api/scheduled-job-candidates` 列表、详情、编辑、启用和拒绝接口，候选写操作使用乐观版本号并限制为管理员；候选来源、所有者和审计字段均保持只读。历史来文缺少上传账号时，候选以空所有者保留并禁止启用，避免丢失人工待确认事项。
- 来文管理新增归档与删除保护：默认列表排除归档项并支持 `include_archived` 显式查询，详情返回候选数、已启用任务数和删除/归档能力字段；已确认、已启用或已有候选审计引用的来文拒绝物理删除并返回稳定 `incoming_has_audit_reference`，草稿删除按候选、批次、抽取记录和来文的外键依赖逆序执行。
- 用户软删除改由生命周期服务在一个数据库事务内完成：停用账号和 API Key 的同时取消用户拥有的活动、暂停定时任务并写入 `owner_deleted` 审计，既有运行和收件箱历史保持可追溯。

### 定时任务（阶段 1，个人 HTTP 接口）

- 新增个人定时任务 HTTP 接口：创建请求强制幂等键，列表按视图提供不透明游标分页，暂停、恢复和取消统一通过状态命令执行并使用乐观版本号；取消原因同步写入审计记录。
- 修复认证读取已自动开启数据库事务后，定时任务、候选审核和收件箱写接口再次开启事务导致的 500：各接口改为复用当前请求会话，在服务成功后提交、领域错误后回滚，保持跨表写入的原子性。新增真实 HTTP 生命周期 E2E，覆盖预览、创建、幂等重放与冲突、列表、编辑、未读计数和取消。
- 新增统一收件箱接口：通知按接收人事件分页，任务按所有者任务聚合展示；通知动作的投递异常归入所有者的通知页签，任务页签只聚合 Agent 执行状态，正常通知不会在两个页签重复出现。未读数量将通知按条目、任务按不同任务分别统计，所有已读操作均按当前认证用户限定并可重复调用。通知游标的未读优先级改用 SQL `CASE` 计算，避免 ORM 对 Boolean 的大小比较在第二页查询时报错。
- 修复 Scheduler 与 Dispatcher 健康探针初始化应用级连接池导致 PostgreSQL 连接耗尽的问题；探针改为使用单个、带超时且显式关闭的 `asyncpg` 连接查询实例心跳。
...
- 将 `croniter>=6.2.2,<7` 声明为后端直接依赖；Cron 时间计算后续只复用该成熟库和标准库 `zoneinfo`，不自行实现 Cron 或时区算法。借鉴来源、许可证与采用边界记录在 `docs/vibe/2026-08-05-来文定时任务模块调研与设计.md`。
...
- 继续多级压缩重构：L3 只外置当前请求内较早 API round 的安全工具载荷并保护最近两个闭合 round；L5 以完整 API round 生成累计 checkpoint，保护最新用户原文及其所在 round。默认摘要模板采用九维结构，管理员 `summary_prompt` 可定义其他字段和输出结构；框架统一追加累计合并、输入隔离、精确值保护和输出预算协议。消息、上一版 checkpoint、候选摘要及修复控制块都只作为历史数据，不能改变摘要任务。长期要求只有在用户明确取消、替换或更正时才失效，失效项保留精确标识和变更状态；待办只有在存在可靠工具结果、交付物、其他可核验证据，或用户明确确认时才转入进展，助手仅声称已记录或回复不代表完成。恢复到主模型时以最新真实用户消息确定本轮任务和回答形式，同时遵守未被明确变更的长期约束；旧 `current work`、`next step` 和临时执行方式仅在仍与当前消息一致时继续。超大单轮分块只在摘要调用内重复原始 HumanMessage，不复制图片或工具正文。精确锚点只从上一版 checkpoint、被压缩的 HumanMessage 和代码注入的用户锚点提取；候选新造值不会进入当次修复清单，但候选通过并提交后会在下一次 L5 作为上一版 checkpoint 被信任。候选遗漏锚点时只修复一次，修复仍遗漏、输出截断、超限或生成失败均不提交 checkpoint。摘要输入预算和输出 cap 按 32K～256K 部署窗口统一动态计算；provider PTL 只按完整 API round 收紧后重试一次。完整历史保存在 `/outputs/conversation_history/`，检查点缺少细节、内容冲突或需要逐字核对时必须先回读，不得猜测；活跃 Skill 的权威 `SKILL.md` 路径及 Todo、附件、artifact、工具/MCP 等独立状态继续保留。
...
- 修复 chat-iframe 小助手重新打开会话列表后回到顶部的问题：抽屉挂载或会话列表刷新完成时，会将当前选中会话定位为顶部第一条可见项，并保留列表原有的顶部间距；重新打开抽屉会复用已加载的分页数据，避免“加载更多”后选中的会话被首屏刷新移除。
- 优化主站与 chat-iframe 小助手会话列表的长标题识别：标题仅在实际被截断时于悬停或键盘聚焦后展示完整名称和更新时间，短标题保持安静；主站列表项为右侧置顶与更多操作预留稳定空间，iframe 浮层根据抽屉外侧空间自适应定位，避免操作区或抽屉裁剪导致信息不可见；两端提示统一使用贴合浅色主题的白色信息浮层、弱边框和轻量阴影。
...
### 离线可视化
- 新增统一 Agent artifact 交付协议：`present_artifacts`、三类可视化工具和 `export_office_file` 统一通过内部 helper 返回一个 `Command`，同时更新实时 `state.artifacts` 和不进入模型上下文的 `ToolMessage.artifact`；聊天保存链路只按该协议归属本轮交付物，不再枚举工具名称、解析 `artifact_path`、判断扩展名或根据 `write_file` 回执推断。子智能体完成后会把最后一轮交付协议转发到父工具消息，续跑不会重新发布旧文件；`task.description` 同时通过 Schema 与执行入口拒绝少于 20 个字符的占位简报。全局提示和所属 Skill 统一约束：自动交付工具成功后直接回答，通用最终文件仍显式调用 `present_artifacts`，中间文件不交付，同一文件不得重复交付；唯一的文件交付章节集中在提示末尾，要求保留用户指定名称原文，并明确 `write_file`、`edit_file` 只完成写入，最终文件必须紧接着显式交付。chat-iframe 的工具调用组在运行期间自动展开、全部完成后自动收起，避免长任务过程淹没正式回答。工具摘要统一按用户可见调用计数，Skill 激活不再重复计入普通工具；单调用直接显示调用行，点击后展开参数和结果，仅同一消息包含多条调用时保留分组摘要。常用内置工具显示中文名称，并为折叠控件补充展开状态。

- 新增离线可视化能力包：通过按需激活的中文 Skill 生成 CSV 数据图表、流程图和思维导图，渲染结果统一为 SVG 并复用现有 artifact 预览、下载与保存链路。渲染工具不进入 Agent 默认工具集，新用户请求会清空上一请求激活的专用 Skill；中间 CSV 和结构文件不会被自动交付。数据图表继续使用固定版本 ECharts SSR，流程图由受限 JSON 转换为固定版本 D2，思维导图由固定版本 AntV Hierarchy 计算双向、节点尺寸感知的 XMind 风格布局，再通过 ECharts SSR 输出多分支主题 SVG；原有 Charts MCP 仍保留为内置默认项，不与新的按需可视化 Skill 绑定。API/worker 镜像构建阶段安装 ECharts、AntV Hierarchy、csv-parse、D2、resvg 和中文字体；项目沙盒运行时镜像固定安装 PyMySQL，MySQL 数据准备在禁网环境不再触发依赖下载。
- SVG 交付物现在通过鉴权 artifact 接口直接显示在聊天列表，保留原有预览和下载操作；会话内全部 SVG 都会加载内联图像，不再只显示最近三项。大型缩略图使用稳定预览高度并可直接打开完整预览，完整预览使用更大的视口并默认按比例显示整张静态图，避免宽流程图或思维导图打开后只显示局部分支。思维导图按中英文实际显示宽度自动换行，一级分支按子树规模分配到中心主题两侧，标签尺寸参与布局避让并使用曲线分支；XLSX 中含图片的展示型工作表按图片方向设置 A4 单页适配，大数据表仅限制为一页宽。下载交付物时临时链接会挂载到页面并延迟释放 Blob，兼容内置 Chromium 的异步下载处理；切换会话或卸载组件时仍会释放预览所用临时 Blob URL。
...
- 三类可视化子 Skill 的 `output_name` 支持安全的中文文件名，用户明确指定名称时必须保留原名，不再强制翻译为英文；路径分隔符、扩展名、空格等不安全字符仍由服务端拒绝。思维导图进一步取消无复用价值的 `.mindmap.md` 中间文件，公开工具名调整为更易分词的 `render_mind_map`，直接接收不超过 8,000 字符、100 个节点且单节点不超过 48 字符的 Markdown 大纲并自动交付 SVG。Skill 同步给出完整首次调用参数模板，要求直接读取 Skill、禁止先列目录或调用文件/命令工具，避免本地模型猜测路径参数、漏写工具名字符后反复修正。
- 修复可视化运行结束时最终状态中的新增交付物未即时合并到流式消息的问题；异步运行的精简 SSE 也会保留 `finished.presented_artifacts`，即使消息历史仍在写库，SVG 卡片也会在当前回答完成后直接显示，无需刷新页面。
- 明确思维导图中心主题也必须使用无序列表项而不是 Markdown 标题；渲染失败时优先返回 Node.js/Python 的真实异常行，不再用运行时版本尾行覆盖可执行错误。

### 上下文管理

- 新增可配置的真实会话 API 验收器：JSON 场景可创建多个隔离会话，并在每个会话内串行发送多轮问题；每轮以回复文本/正则、工具调用、文件结果、压缩 SSE 事件和运行状态进行可重复的规则校验，最终输出完整汇总。首期不把本地模型裁判作为放行条件，避免模型随机性掩盖回归；聊天记录、工具步骤和产物会保留在 chat-iframe 对应会话中供人工复核。
- Pytest 默认改用 `importlib` 导入模式，隔离 `unit` 与 `integration` 中同名测试模块，确保项目规定的统一 `pytest test` 入口能够完整收集所有测试。
...
- 收紧已完成工具参数归档：只有替换后可释放至少 128 个估算 Token 的大正文或大 JSON 参数才写入 `large_tool_results`，普通结构化参数保持原始 schema，避免本地模型仿写内部归档占位字段并产生无效工具调用。
...
- 修复小助手运行状态与交付物同步：chat-iframe 在流式期间按主站节奏补拉线程状态，且仅在本轮 `write_todos` 改变待办后显示“本轮进度”，不会把持久化的上一轮待办误显示为新任务。删除上下文面板重复的“模型格式等额外开销”文字项，保留进度条中的“模型协议/模板校正”作为唯一解释；工具结果外置会单独说明已收纳数量，明确它不是历史摘要。交付物只接受成功工具结果携带的标准 artifact 协议，不扫描 outputs，也不根据 `write_file`、`edit_file`、工具缓存或历史文件回执推断。
...
- 新增原生 Office 导出能力：`office-export` 按格式提供 DOCX、PDF、XLSX references，业务 Skill 只通过依赖名读取其入口，不耦合安装路径或 reference 文件名；`export_office_file` 使用 `ToolRuntime` 安全解析当前会话 definition 和图片路径。DOCX/PDF 支持标题、段落、表格、图片和分页，纵向长图会按页面可用宽高等比缩小并为题注预留空间，分页要求附着到后续内容块以避免长图占满页面时产生空白页；XLSX 支持多工作表、冻结表头和单元格锚点图片。SVG 仅在写入 Office 文件时通过本地 resvg 临时转换为 PNG，最终文件直接写入当前线程 outputs；ECharts、csv-parse 和 resvg 的依赖清单上移到后端运行时，不再由可视化 Skill 目录承载。Office 与 PDF 交付物使用随 chat-iframe 离线打包的 PDF.js 连续渲染，支持多页滚动、缩放和适合窗口，并为 `.mjs` worker 静态资源声明浏览器可执行的 MIME 类型，不依赖宿主浏览器 PDF 插件。旧的内置导出 MCP、临时产物拦截器和格式专用入口已删除，通用的内置 MCP 默认配置注册与数据库同步机制继续保留。
...
- 增强 chat-iframe 高级运行体验：活动会话在消息流中实时展示工具调用和 Todo 进度，重新进入运行中的会话会恢复后端 state；输入区模型选择左侧按需显示最近一次模型调用的上下文用量；Agent 通过 `present_artifacts` 声明的交付物会持久化到最终回答元数据，刷新后仍显示在原回答下方，并支持带鉴权的图片、PDF、文本和 Office 预览及原文件下载；DOC/XLS/PPT 家族仅在用户点击预览时由后端临时转换为 PDF。
...
- 补强 L2 历史工具参数投影的 API 验收场景：以两轮普通消息稳定累积压力，并校验场景轮次顺序。
<!-- 添加到这里 -->
