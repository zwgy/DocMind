...
- 调整来文摘要提示和接口契约：`/api/incoming-documents/ingest` 固定使用 docMind 项目字段，外部系统需在接入侧映射为 `sourceUrl/sourceKey/sourceDocId/filename/file/metadata`；摘要提示明确分类候选集合，并将 `structured_result` 定义为摘要阶段轻量关键事实，不替代正式业务结构化抽取结果。
