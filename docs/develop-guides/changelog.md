...
- 流程图改为在 `render_flowchart` 调用中直接传入节点和连线，移除容易被本地模型写坏的中间 JSON 文件，并禁止为代写流程定义而调用子智能体；默认只绘制用户明确要求的 2 至 10 个节点，不扩展整篇材料；外部节点 ID 改为渲染前映射的任意短文本，判断节点支持两个及以上分支，渲染器一次返回全部拓扑错误，同时保留连通性与 SVG 安全校验。工具 Schema 明确边只能原样引用节点 ID、`output_name` 只能位于顶层；无效边会同时返回具体引用和有效 ID，减少本地模型重复猜测参数。
