...
- 修复运行时提示词硬编码 Skill 或工具使用方式的问题：chat-iframe 的来文上下文和文档摘要均按 `incomingId` 聚合，不同来文独立分块，同一来文共用分类和元数据；主附件展示摘要与结构化结果，副附件仅展示名称与摘要并以分隔线区分；技能说明、按需激活与工具开放统一由 Agent 的 SkillsMiddleware 和对应 `SKILL.md` 处理，避免 Agent 未配置能力时收到不可执行的指令。
