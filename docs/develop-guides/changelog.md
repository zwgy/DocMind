...
- 修复本地模型在多次工具调用后空白结束的问题：模型缓存的完整窗口、最大输出和安全缓冲会写入 LangChain profile，主 Agent 与子 Agent 在最终请求超出可用输入预算时再压缩，而不是按窗口百分比提前触发；模型配置的最大输出同时限制实际 provider 请求，调用方只能进一步收紧，避免预算预留与真实生成上限不一致；OpenAI 兼容服务返回空正文且 `finish_reason=length` 时转为标准上下文溢出并进入恢复流程。来文原文模式不再重复返回整套结构化结果，已知来文与附件 ID 时直接将 Markdown 写入当前线程 outputs，并由 `present_artifacts` 交付，不再为单纯文件交付把全文读入模型上下文。
