...
- 完善未入库附件的嵌入式同步闭环：chat-iframe 发现 `pending_sync` 后通过通用消息协议通知父页面 SDK；SDK 使用接入方提供的 `source_url` 下载附件并上传现有 DocMind 入库接口，同时回传下载、上传、完成或失败状态。聊天区顶部在可选通知轮播下方持续展示准备进度，解析未完成时保留用户输入并提示等待，解析就绪后自动刷新摘要并提示可提问；上传响应不确定时先以后端查询结果对账，避免重复上传。最小化或关闭悬浮窗不会中断状态；宿主 SPA 以 `source_system + source_function_id + business_id` 识别页面，通过 `setPageContext()` 切页时中止并清空旧 SDK 任务、重载 iframe 清空旧状态；接入方只需用 `source_file_id` 标识页面附件，SDK 使用当前 `business_id` 自动补齐后端文档级标识。同一批附件复用进行中的任务，失败后允许重试，旧 SDK 与非嵌入场景继续使用原有直传路径。本次不增加特定系统适配器、自定义 `fileLoader` 或新的后端接口。
- 修正嵌入附件范围与来文元数据示例：同一来文仍按完整附件集合完成后台同步和解析，但模型上下文只包含用户本轮实际勾选的附件；`example.html` 统一使用 SDK 支持的 `document_metadata` 参数，避免新入库来文因标题元数据为空而回退显示业务 ID。
