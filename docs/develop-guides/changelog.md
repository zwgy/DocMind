...
- 修复周期任务反问后的恢复创建：结构化 `ask_user_question` 回答以 `ToolMessage` 回放，创建工具以最新 `HumanMessage` 隔离当前请求，只按稳定的 `question_id=scheduled_task_time` 读取其后的回答映射，不再匹配问题文案或语言；旧轮次、无关问题、非结构化和空回答均不会放宽时间校验。
