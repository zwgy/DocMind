...
- SVG 交付物现在通过鉴权 artifact 接口直接显示在聊天列表，保留原有预览和下载操作；大型缩略图使用稳定预览高度并可直接打开完整预览，完整预览使用更大的视口并默认按比例显示整张静态图，避免宽流程图或思维导图打开后只显示局部分支。思维导图按中英文实际显示宽度自动换行，一级分支按子树规模分配到中心主题两侧，标签尺寸参与布局避让并使用曲线分支；XLSX 中含图片的展示型工作表按图片方向设置 A4 单页适配，大数据表仅限制为一页宽。下载交付物时临时链接会挂载到页面并延迟释放 Blob，兼容内置 Chromium 的异步下载处理；切换会话或卸载组件时仍会释放预览所用临时 Blob URL。
...
- 三类可视化子 Skill 在主流程中显式约束 `output_name` 使用 ASCII 文件名主体；思维导图进一步取消无复用价值的 `.mindmap.md` 中间文件，公开工具名调整为更易分词的 `render_mind_map`，直接接收不超过 8,000 字符、100 个节点且单节点不超过 48 字符的 Markdown 大纲并自动交付 SVG。Skill 同步给出完整首次调用参数模板，要求直接读取 Skill、禁止先列目录或调用文件/命令工具，避免本地模型猜测路径参数、漏写工具名字符后反复修正。
