...
- 优化 chat-iframe 的个人定时任务体验：邮箱图标调整为时钟入口，单一“定时中心”默认展示当前认证用户的“我的定时”，支持按进行中、已暂停和历史查看通知与 Agent 定义，并直接编辑、暂停、恢复或取消；收件箱改为仅展示已发生事件，通知页包含已投递和投递异常，任务页只聚合 `agent` 动作的执行状态。时钟红点仅同步收件箱未读事件，不把待触发任务计为未读。定时中心标题改为稳定居中，“我的定时”与“收件箱”的二级页签统一布局和选中效果；任务卡片补充通知正文或 Agent 指令，常见 Cron 转换为“每周五 17:00”“工作日 09:00”等可读描述，复杂表达式保留原值以避免错误解释。
