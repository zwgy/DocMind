...
- 修复 chat-iframe 来文上下文把 `incoming-document` Skill 的名称、激活步骤和使用方式硬编码进提示词的问题：iframe 层现在只提供来文摘要和原文定位参数，技能说明、按需激活与工具开放统一由 Agent 的 SkillsMiddleware 和对应 `SKILL.md` 处理，避免 Agent 未配置该 Skill 时收到不可执行的指令。
