...
- 优化 chat-iframe 的个人定时任务体验：邮箱图标调整为时钟入口，单一“定时中心”默认展示当前认证用户的“我的定时”，支持按进行中、已暂停和历史查看通知与 Agent 定义，并直接编辑、暂停、恢复或取消；收件箱改为仅展示已发生事件，通知页包含已投递和投递异常，任务页只聚合 `agent` 动作的执行状态。时钟红点仅同步收件箱未读事件，不把待触发任务计为未读。一级页签保持紧凑左对齐，两组二级页签统一布局和选中效果；“全部已读”仅在当前分类存在未读事件时显示，空列表按当前页签提供图形和准确文案。小助手与 Web 的任务列表同步补充通知正文或 Agent 指令、动作摘要、可读调度规则及下一次/最近触发时间；常见 Cron 转换为“每周五 17:00”“工作日 09:00”等描述，复杂表达式保留原值。时区继续参与调度计算并在必要的 Web 编辑配置中保留，但不进入列表、卡片或详情展示。
