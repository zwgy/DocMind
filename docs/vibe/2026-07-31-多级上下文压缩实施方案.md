...
9 个维度是默认摘要 prompt 的内容要求，不是框架不可覆盖的序列化协议。管理员 `summary_prompt` 完整定义自定义摘要的字段和输出结构；框架只追加与字段结构无关的累计合并协议并绑定本次输出预算。累计协议统一规定：`previous_summary` 是累计 checkpoint，新消息默认是增量而不是覆盖；必须先继承所有仍有效的旧约束、决策、标识和待办，再吸收新事实；最近消息更具体本身不能让旧项完成或失效；只有新增消息明确表达取消、替换、更正、不再需要或完成时才能更新旧项，完成事项应从待办转入进展并保留仍有连续性价值的结果。默认九维策略缺少标题或顺序变化时只记录质量指标；自定义策略记录为 `custom`，两者都不为修复外观格式额外调用模型。代码统一校验非空、非 API 错误、未发生输出截断以及最终预算可容纳。
...
- `AC-L515` 管理员 `summary_prompt` 可以完整替换默认九维输出结构；框架只追加结构无关的累计合并与预算协议，不暗中恢复九维字段。
