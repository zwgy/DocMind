...
> 2026-09-08 更新：本文关于“摘要失败终止、无摘要不裁剪”和信任上一版摘要精确值的约束已由 [上下文压缩连续会话实施方案](./2026-09-08-上下文压缩连续会话实施方案.md) 取代；其他架构边界继续适用。

> 日期：2026-07-31
> 状态：实施完成，验收通过；功能、真实模型、API 场景、整库 Integration、E2E 与统一全量测试入口均已验证
...
| 摘要输出可能膨胀或被字符前缀截断 | 提示词中的 token 要求不是模型输出硬限制，前缀截断会先丢尾部 NEXT STEPS | 给摘要调用设置真实输出上限；要求结构化 checkpoint；provider 截断或超限时不提交候选摘要 |
...
| 摘要模型失败时可能重复尝试或提交半成品 | 缺少分级提交和有限重试契约 | 归档、活动视图和 checkpoint 在主模型成功后原子提交；每个有界分块非 PTL 失败不重试，整次滚动摘要最多进行一轮 PTL 恢复 |
...
8. Compact 调用不绑定 Agent 工具，并设置真实摘要输出上限；Claude Code 的 compact 路径没有提供可移植到所有 OpenAI 兼容服务的统一 reasoning 开关。
...
单个工具结果的快速外置阈值不再作为 Agent 管理参数。主 Agent、SubAgent、`read_file` 和
`open_kb_document` 必须复用同一个运行时解析值，旧 Agent JSON 中残留的 `tool_token_limit`
直接忽略。原因是该阈值属于上下文基础设施策略，而不是不同 Agent 应自行改变的业务行为。

解析公式为：

```text
tool_inline_limit = clamp(
    floor(prompt_budget / 16),
    3 * 1024,
    16 * 1024,
)
```

这里使用模型 profile 的部署级 `prompt_budget`，即完整窗口扣除最低输出预留和安全缓冲；
单次请求显式扩大输出上限时，最终请求仍由 L1～L5 按该次更小的 `prompt_budget` 重新收敛。
单项阈值只是工具执行后的快速保护，不能代替最终请求预算门禁。

`/16` 不是 Claude Code 源码中的比例常量。Claude Code 可验证实现仍以整体上下文压力和工具
调用新旧边界清理工具结果，没有可直接复制的窗口比例公式。本项目比较 `/24`、`/16`、`/12`
后选择 `/16`：按最近两个闭合 round 中常见的四个大结果估算，64K 以上四项合计最多占提示
预算约 25%；`/12` 会放大到约 33%，容易挤压 system、Skill 和工具 schema，`/24` 的约 17%
又会让大窗口过早外置。工具数量异常多时仍由整体预算循环处理。

按最低输出预留 4096、安全缓冲 1024 的预算边界测试：

| 部署窗口 | `prompt_budget` | 自动单项上限 |
| --- | ---: | ---: |
| 32K | 27,648 | 3,072 |
| 64K | 60,416 | 3,776 |
| 128K | 125,952 | 7,872 |
...
32K 的 3K 下限沿用既有真实 API 场景验证过的保守边界。64K～256K 的数值已通过真实中间件、
模型 profile 和外置边界单测验证，但在对应窗口的本地模型实际部署可用前，只能视为预算与数据
形状验收，不能宣称已经验证回答质量。部署对应模型后应复用 API 载荷场景比较外置次数、L2/L3/L5
触发情况、回答连续性和重读次数，再决定是否调整除数或上下限。
...
```text
deployment_summary_cap =
    min(
      20000,
      max(min_output_reserve_tokens, floor(context_window / 8))
    )

summary_output_limit =
    min(
      当前最终请求可用于 checkpoint 的 token,
      deployment_summary_cap,
      context_window - safety - 摘要协议 - 最小下一分块
    )
```

在常用 4K 最低输出预留下，同一公式使 32K、64K、128K、256K 的部署 cap 约为 4K、8K、16K、20K。它不是按模型名或参数规模判断能力，也不猜 provider 未公开的独立最大输出；20K 只沿用 Claude Code 的 compact 输出封顶，窗口八分之一用于防止把该固定值直接套到 32K 本地模型。
...
摘要输入超过该值时，先按协议安全消息段和有界文本块滚动合并，每次调用都生成完整替代 checkpoint，不会把九个维度拆成九次请求。若 provider 仍返回 prompt too long，再按完整 API round 移除最旧约 20% 输入、把输入估算收紧到 75%，并把整次滚动摘要重试一次；仍失败则返回明确压缩错误，不提交摘要或消息裁剪。
...
- 单结果上限按 6.5 的模型提示预算自动解析，不接受 Agent 级手工覆盖。
...
flowchart TD
    ...
    C --> D["构建有界摘要输入<br/>S(n) + C(n+1) 的 L1/L2/L3 投影视图<br/>按摘要输出 cap 反算独立输入预算"]
    D --> E["调用摘要模型<br/>不绑定 Agent 工具，设置真实 max_tokens<br/>一次生成完整九维候选摘要 S(n+1)"]
    E --> F{"Provider 调用结果？"}
...
L5 最终只生成一份九维语义 checkpoint。每次摘要模型调用都生成完整九维替代结果，不是把九个维度分别调用九次模型；历史超过单次摘要输入预算时，可以通过多个有界分块滚动更新同一份 checkpoint：
...
9 个维度是默认摘要 prompt 的内容要求，不是框架不可覆盖的序列化协议。管理员 `summary_prompt` 完整定义自定义摘要的字段和输出结构；框架另外追加与字段结构无关的累计合并、输入隔离、精确值保护和输出预算协议。所有消息、上一版 checkpoint 和候选摘要都作为待整理数据，不能改变摘要任务或输出结构。`previous_summary` 为 `None` 表示首次生成，否则表示上一版累计 checkpoint；新消息只做增量合并。长期要求只有在用户明确取消、替换或更正时才失效，失效项仍保留精确标识和变更状态；待办只有在存在可靠工具结果、交付物、其他可核验证据，或用户明确确认时才转入进展，助手仅声称已记录或已回复不代表完成。默认九维策略缺少标题或顺序变化时只记录质量指标；自定义策略记录为 `custom`，两者都不为修复外观格式额外调用模型。代码统一校验非空、非 API 错误、未发生输出截断以及最终预算可容纳。
...
字段优先级用于约束摘要模型在输出 cap 内主动压缩低优先级叙述，不是代码侧二次裁剪算法。若 provider 仍返回 `finish_reason=length`、实测/估算输出超过 cap，或候选摘要使最终请求超预算，本次 L5 明确失败并保留旧 state；代码不再截取字符串前 N 个字符，也不对九维字段做可能改变事实的自动删减。
...
latest Human 不送入摘要模型，也不要求写进第 6 项，因为它会以真实消息留在最终请求中，并在最终 system checkpoint 之后成为更高优先级的任务来源。能够一次容纳时由一次调用生成 `S1`；过长时按有界分块滚动生成完整替代结果，最后一份结果为 `S1`。原消息归档为 `A1`，主模型成功后状态变成：
...
能够一次容纳时由一次调用生成完整替代摘要；过长时仍滚动更新，最终结果记为 `S2`：
...
3. 本地预估超限时按有界分块滚动合并，不发送超过摘要输入预算的请求；provider 仍返回 PTL 时，按完整 API round 移除最旧约 20% 输入并把估算输入预算收紧到 75%。
...
摘要直接调用当前 BaseChatModel，不经过 Agent 工具绑定，因此不会携带工具 schema。首期不增加 provider-specific thinking/reasoning 参数分支：不同本地 OpenAI 兼容服务的开关名称和 usage 口径不一致，错误关闭反而可能使请求失效；reasoning token 由真实输出上限与 provider usage 一并约束。摘要为空、API 错误、输出截断、重试后仍 PTL 或最终请求无法容纳时，保留旧 LangGraph state 并返回明确错误；不得由代码推测用户目标并生成 degraded checkpoint。
...
- 本地预估先收敛摘要输入，不消耗模型调用。
- 每个本地预算内分块只调用一次；非 PTL 失败不重试，provider 明确 PTL 时只允许把整次滚动摘要重跑一轮。
...
- 对当前轮已读取并激活的 Skill，恢复其 slug 和权威 `SKILL.md` 路径，并明确要求继续受约束步骤前重读。
- 不复制 Skill 正文。多个 Skill 的正文可能直接耗尽 32K 窗口，而权威文件仍可通过现有文件工具读取；路径指针比摘要模型复述或截断指令更可靠。
...
`archive_ref` 只用于代码校验、幂等写入、测试和诊断，不直接要求模型解析。模型请求中只注入最新 `archive_path` 和统一回查说明；当历史用户要求、精确标识/路径、既往决策或工具结果不确定时，必须先通过现有 `ls/read_file(path, offset, limit)` 分段读取原始 JSONL，再继续行动或回答。更旧 revision 的路径从 `/outputs/conversation_history/` 查找，不能猜测内容；常规明确请求不无条件读取归档，避免增加无效工具调用和上下文占用。
...
3. `finish_reason=length` 且工具调用 JSON/参数不完整
   ...
   - 在 ToolNode 前抛出明确的不可提交输出错误，绝不执行该工具调用，也不交给普通 ModelRetry。
4. `finish_reason=length` 且无可见正文
   - 本地 Qwen 可能把输出额度消耗在 thinking/reasoning token。
   - provider 的正数 output/reasoning usage 能证明输出额度耗尽时，仍按 `output_exhausted` 处理，但返回可操作错误，不提交空 AIMessage。
   - 若 provider input 已超过本地 `prompt_budget`，说明准入估算或部署配置不一致：先用实测 input 校准，再进行一次恢复性压缩。
   - usage 缺失时分类为 `length_unverified`，返回明确的“输出被截断且原因无法校验”，不默认伪装成输入溢出。
...
- P1 已区分 `input_overflow`、`output_exhausted` 和 `tool_call_truncated`；P6 再补齐空正文无 usage 的 `length_unverified`，并阻止截断工具调用和空 AIMessage 进入 LangGraph state。只有实测 provider input 超过本地 `prompt_budget` 才触发一次历史压缩恢复。
...
- 摘要直接调用当前模型且不绑定 Agent 工具；不增加 provider-specific reasoning 参数分支。
- 摘要输入使用 API-round 边界，本地先收敛；provider PTL 时最多重试一次。
- 删除字符前缀截断；provider 截断、超限或最终请求仍不可容纳时明确失败。
- 恢复 active Skill 的 slug/权威路径；附件、Todo、工具/MCP 和 artifact 等独立 state 不被覆盖。
...
- 摘要输入与输出已在同一部署窗口内闭合计算。输出 cap 使用同一公式覆盖 32K、64K、128K、256K，分别约为 4K、8K、16K、20K，且不按模型名称或参数规模分支。
- provider PTL 只允许一次恢复：按完整 API round 移除最旧约 20% 摘要输入，并将重试输入估算收紧到原预算的 75%；第二次 PTL、普通摘要异常、`finish_reason=length` 和超限输出都不提交 checkpoint。
- 已删除摘要字符前缀截断路径。九维标签不完整只标记 `format_unverified`，不额外调用模型修复；provider 返回的正数 output usage 优先用于输出校验，缺失、零值或不可信值回退本地保守估算。
- L5 恢复段保留 `activated_skills` 的 slug 和权威 `SKILL.md` 路径；`messages` 的 `Overwrite` 不覆盖 Skill、Todo、附件、artifact 等独立 state。
- 同步/异步失败事件已区分归档失败、摘要 PTL、摘要输出截断、摘要输出过大和普通摘要失败，并完整记录未提交状态下的 Token、消息、round 与归档计数，不包含异常正文。
- 本地与 `docmind-dev` API Docker 容器的 context projection、context compaction、LangGraph stream 和 token usage 回归共 74 项通过；Ruff 与格式检查通过。远端 HEAD 为 `3f2d099d`，API 健康、worker 运行正常；本阶段仅修改热挂载后端源码和文档，无需重建镜像或重启服务。
...
验收记录（2026-07-31，P6.1）：

- L5 已通过 LangGraph custom event 输出不含内容正文的 `context_compaction` 生命周期事件：开始事件带 `level`、`reason`、`tokens_before`，完成事件补充 `tokens_after`、`messages_removed`、`rounds_removed` 与 `archive_count`。现有实现已区分 proactive admission 和 provider overflow；summary PTL/failure 的细分事件将在摘要恢复分支完成后验收。
- 本地回归与 `docmind-dev` API Docker 容器均执行 context projection、context compaction、stream 和 token usage 测试，58 项全部通过；Ruff 检查与格式检查通过。远端 HEAD 为 `207b4aa6`，服务健康且无需重启容器。
- 正式文档和 changelog

验收记录（2026-08-03，P6.2）：

- `TokenUsageMiddleware` 已区分 `input_overflow`、有正文/有 usage 的 `output_exhausted`、`tool_call_truncated` 和无 usage 的 `length_unverified`。截断工具调用在真实 `create_agent` 的 ToolNode 前终止，空正文不进入消息状态，容量异常不交给普通 `ModelRetry`。
- 真实 LangGraph 中间件栈验证 outer ContextCompaction `Overwrite` 与 inner TokenUsage `Command` 同时保留；实测 input overflow 只恢复调用主模型一次，总调用数为 2。主 Agent 与 SubAgent 均只注册一个压缩器，顺序为 ContextCompaction → TokenUsage → ModelRetry。
- 首轮真实 32K Qwen 质量探针发现：旧默认模板的六段结构与固定九维协议冲突，而且超大单 Human turn 的后续摘要块会遗忘最早用户消息。实现因此统一为九维模板，并仅在滚动摘要调用内给每个分块重复该段原始 HumanMessage；该锚点不进入 checkpoint、主模型请求或持久状态，不复制工具正文。
- `docmind-dev` 的 Ollama `/api/ps` 确认 `qwen3.6:35b` 实际加载窗口为 32,768。最终合成探针包含 1 条用户请求、30 个闭合工具 round、总计 61 条消息和约 10K 文档级工具文本；首次和再次 L5 的 4,096 输出 cap 均生效，checkpoint 分别估算为 960、1,076 token，九维质量均为 `semantic`。两次摘要都保留“不实现 L4”、`/outputs/context_projection.py`、`ERR-CONTEXT-417`、Docker 验收、重启待办，并吸收后续 P5/P6 与截断工具纠正。该探针使用真实模型验证 L5 与再次 L5，不等同于带登录、数据库和 SSE 的 HTTP E2E。
- 第二窗口验收使用同一 Ollama 主机上的 `qwen3.6:27b`，先通过原生 `/api/chat` 以 `num_ctx=65536` 加载，并由 `/api/ps` 确认实际 `context_length=65,536`，不把 OpenAI 兼容接口忽略 `options.num_ctx` 的结果当作大窗口证据。探针包含 1 条用户请求、30 个闭合工具 round、61 条消息，保守估算为 64,690 token，超过 60,416 的主请求预算；首次 L5 因而真实滚动为 2 次模型调用，provider 输入分别为 40,616 和 7,131 token，再次 L5 为 1 次调用、输入 1,227 token。三次调用均以 `stop` 完成，8,192 summary cap 生效，首次和再次 checkpoint 分别估算为 626、815 token且九维质量均为 `semantic`；两次都保留 L4 禁令、精确路径、错误码和 Docker 事项，再次摘要继续保留 P5/P6、`tool_call_truncated`、`BUILD-64K-PASSED` 与目标分支。验收后已卸载 27B，并重新加载 `qwen3.6:35b`；`/api/ps` 确认环境恢复为唯一 35B 实例和 32,768 窗口。
- Docker 相关回归共 83 项通过，覆盖 1000 轮有界 checkpoint、30 round、32K/64K/128K/256K 同算法、同步/异步、主/SubAgent、Skill 状态、一次 overflow 和原子提交。全量 unit 在常驻 API 容器为 1,068 passed、2 skipped，10 个只因容器未挂载仓库根文件失败；同一 API 镜像完整只读挂载补测这 10 项全部通过，合计 1,078 passed、2 skipped、0 failed。Ruff 与格式检查通过。
- 2026-08-03 使用专用管理员测试凭据在已部署提交 `b0eae98e` 上复验：上下文压缩确定性矩阵 97 项全部通过，覆盖 L1/L2/L3/L5、32K/64K/128K/256K、1000 轮、单 Human 30 round、多 Skill/MCP、主/SubAgent、归档、再次摘要、一次 overflow 恢复和失败原子性。真实 HTTP E2E 为 7 passed，覆盖 Agent Run、排队/取消、附件与状态、子智能体流和 Viewer 文件系统。
- 同轮全量 Integration 为 141 passed、7 skipped、9 failed。9 个失败均位于本分支未修改的知识库/Viewer 测试：7 项使用部署中已经不受支持的 `siliconflow-cn:Pro/BAAI/bge-m3` 创建知识库，2 项仍把 Viewer 图片/PDF 二进制响应按 JSON 解析；provider connectivity 3 项和批量追问 4 项按各自显式环境条件跳过。本次上下文压缩不能据此记为失败，但全量 Integration 基线修复前也不得宣称整库测试全绿。
- 内置浏览器在 `chat-iframe/example.html` 完成真实小助手验收：新建隔离会话后，模型根据当前页面和已选 PDF 正确回答适用范围、2020-11-01 执行日期与禁止事项，并原样返回 `CTX-IFRAME-0803`；续问不重新读文件即可返回 `CTX-IFRAME-0803｜2020年11月1日`。普通窗/最大化切换、生成中禁用、完成后恢复均正常，页面控制台无 warning/error。
- 新增 `backend/scripts/validate_context_compaction_api.py`，通过 chat-iframe 换票、同一外部用户和业务 scope 直接创建真实线程，逐轮调用 Agent Run 并消费 verbose SSE；脚本只输出诊断字段，消息、工具步骤和文件仍由生产链路持久化，可在小助手会话列表复核。小规模协议试跑用 1 个 1,000 字符增长轮验证鉴权、线程、SSE、`read_file`/`write_file`/`edit_file` 和文件恢复，共 30.7 秒；该试跑允许未触发 L5，不计作压缩通过证据。
- 正式 API 压力验收使用每轮 5,000 字符、最多 10 轮且禁止 `--allow-no-l5`，总耗时 53.5 秒；第 4 个增长轮观察到 `L5 started/finished`，估算输入由 30,886 降至 26,266 token，移除 6 条消息、3 个旧轮次并生成 1 个归档。随后模型仍恢复 `EARLY-CONTEXT-0803-152551-f081a1`、禁止实现 L4、三个 Skill、原文件路径和待办，并写入 `FINAL-CONTEXT-0803-152551-f081a1`。线程 `0a93e538-f7a3-43d3-a81f-fea83b08d50b` 保留在 `oa:incomingDocument:37908`；内置浏览器确认列表可见、最终内容完整，展开“执行过程”可见读取文件和编辑文件两个步骤均完成。
- 正式命令为 `cd backend && uv run python scripts/validate_context_compaction_api.py --base-url http://<host>:5050 --max-growth-rounds 10 --payload-chars 5000 --run-timeout 300`。只有输出包含 `status=passed` 且真实 `L5 finished` 才算通过；`--allow-no-l5` 仅用于排查协议和环境，不得作为上下文压缩验收依据。
- 脚本支持 `--scenario-file <json>`，可在同一外部用户与业务 scope 下创建多个隔离线程，并对每个线程串行发送多轮问题。场景的每轮 `expect` 支持 `assistant_contains` / `assistant_not_contains` / `assistant_matches`、`tools_include` / `tools_exclude`、`files`、`compaction` 和运行状态；脚本逐轮打印输入/输出的有界摘要、SSE 压缩事件、工具、断言结果，并以 `scenario_finished` 输出全量汇总。正文日志默认截断为 1,200 字符，避免长工具结果反过来淹没验收日志。
- `backend/scripts/scenarios/conversation_continuity.json` 是可直接执行的两会话示例。2026-08-03 真实 API 执行共 24 秒：`file-continuity` 会话先以 `write_file` 创建 `CONTRACT-0803-160930-ddbdf8`，第二轮以 `read_file` + `edit_file` 恢复并写入 `DONE-0803-160930-ddbdf8`；`isolated-session` 会话连续两轮均无工具调用，正确保留自身 `ISOLATED-0803-160930-ddbdf8` 标记。四轮规则断言和两条线程的 scope 可见性全部通过。模型裁判不作为首期放行条件，语义需求应优先转写为可重复的文本、正则、工具、文件和 SSE 断言。
- 远端保持 `codex/context-compaction-refactor` 分支。API 与 worker 重启后分别为 healthy/running，`/api/system/health` 返回 200；启动日志未出现本次上下文改动相关错误。
- HTTP E2E、真实 32K/64K 模型探针和小助手浏览器主链路均已完成。测试凭据仅在命令进程中临时注入，没有写入仓库、远端 `.env` 或提交记录。
- 2026-08-03 最终基线收敛：知识库测试改为读取部署当前启用的 embedding 模型，Viewer 图片/PDF 断言修正为二进制响应契约，任务入库测试显式上传真实 MinIO 文件后再入队；Docker 内整库 Integration 为 `150 passed, 7 skipped`，E2E 为 `7 passed`。
- 同日新增的 JSON API 场景验收器在 `docmind-dev` 当前分支复验通过：创建两个 chat-iframe 隔离线程，第一线程连续完成 `write_file -> read_file + edit_file` 并校验文件内容，第二线程连续两轮恢复自身标记且不调用工具；四轮断言、SSE、工具、文件和 scope 可见性均通过。线程 `71b00f8f-7de8-4ef9-98a8-407372d6c831`、`72e72a59-111b-4c7a-9d7d-dbddf7f260e4` 可在对应小助手业务 scope 中回看。
- 统一全量入口使用一次性 Docker 测试容器和只读仓库根目录挂载执行 `pytest test`，最终为 `1239 passed, 9 skipped, 12 warnings`，exit code 为 0。跳过项均是已有的可选外部模型/语义能力或显式环境条件，不包含失败；同名 `unit`/`integration` 测试模块已改用 pytest `importlib` 导入模式隔离收集。
...
- `AC-B01`（已被 2026-08-11 方案替代）缺少实际 `context_window` 时使用 LangChain profile 或系统默认 32768，并在管理页标注来源和默认值风险。
...
- `AC-L109` 32K/64K/128K/256K profile 自动得到同一公式的单项上限；主 Agent、SubAgent、文件和知识文档读取复用该值，旧手工字段不生效。
...
- `AC-L505` 摘要输出设置真实 cap；provider 截断、输出超限或最终请求仍不可容纳时明确失败，不做字段或字符裁剪，不提交候选 checkpoint。
...
- `AC-L515` 管理员 `summary_prompt` 可以完整替换默认九维输出结构；框架只追加结构无关的累计合并、输入隔离、精确值保护与预算协议，不暗中恢复九维字段。
- `AC-L516` 默认九维策略的摘要标题缺失或顺序变化只记录质量指标，不触发格式修复模型调用；自定义策略不按九维标签检查。
- `AC-L517` 默认九维策略的第 6 项覆盖所有被本次摘要消费的非工具用户消息，并继承旧 summary 中仍有效的用户请求。
- `AC-L518` 默认九维策略的一次摘要模型调用生成完整九维结果，不按维度分别调用模型；自定义策略同样只能调用一次模型生成完整替代摘要。
- `AC-L519` 候选摘要通过后必须重新估算完整 `system + messages + tools`；仍超预算时不调用主模型，也不提交候选摘要和消息裁剪。
- `AC-L520` 摘要调用不绑定 Agent 工具；不按 provider 或模型名猜测 thinking/reasoning 开关，reasoning 消耗由真实输出 cap 和 usage 统一约束。
- `AC-L521` 非 PTL 摘要失败不重试；一次 PTL 重试仍失败后立即结束，单次请求不存在无限摘要循环。
- `AC-L522` 消息、上一版 checkpoint、候选摘要和精确值控制块都只作为历史数据，不能改变摘要或修复任务的规则与输出结构。
- `AC-L523` 用户取消、替换或更正旧要求时，checkpoint 保留旧精确标识并标注失效状态；精确值修复不得把失效项重新写成有效要求或待办。
...
- `AC-R06` 空正文 length 且 usage 缺失时分类为 `length_unverified` 并明确报告无法校验，不提交空 AIMessage、不默认触发上下文压缩或普通模型重试。
...
- 最大风险集中在共享协议边界和 L5 的小模型摘要质量；前者由纯函数测试和真实中间件顺序测试控制，后者由原文归档、真实尾部、不绑定 Agent 工具、真实输出 cap 和有界失败控制。
...
- [x] P4 复用共享投影引擎接入当前轮 protected tail 策略。
- [x] P4 32K 下单 Human turn 30 个工具 round 验收通过。
- [x] P5 实现九维 checkpoint 内容要求、真实摘要输出限制、不绑定 Agent 工具和一次 PTL 重试。
- [x] P5 实现摘要失败不提交、非 PTL 不重试和一次 PTL 重试上限。
- [x] P5 实现 active Skills 和运行状态恢复。
- [x] P5 删除字符前缀截断路径。
- [x] P6 完成 provider input overflow、output exhausted、tool truncated 和 usage 缺失分类。
- [x] P6 完成一次 provider overflow 恢复，确认普通 retry 不接管容量错误。
- [x] 完成 unit、真实 32K L5/再次 L5 和 API/worker 重启验收。
- [x] 使用第二个实际 64K 本地模型完成首次/再次 L5 长工具链发布加固验收，并恢复原 35B 32K 环境。
- [x] 使用专用测试凭据完成 HTTP E2E，并在内置浏览器完成 chat-iframe 两轮真实问答。
- [x] 使用真实 chat-iframe 身份和 Agent Run/SSE API 完成可回看的多轮压力验收，观察到 L5 并验证压缩后任务与文件恢复。
- [ ] 修复与本次上下文改动无关的知识库模型 fixture 和 Viewer 二进制断言，使全量 Integration 基线恢复全绿。
- [x] 更新 Middleware、运维、模型配置和 changelog 文档。
- [x] 检查无 L4 占位实现、无并行旧 Middleware、无模型名窗口分支。
...
不为上下文压缩另建 tracing 框架，采用两类互补证据：

1. LangSmith 通过 LangChain/LangGraph 标准环境变量自动记录 Agent、主模型、L5 摘要模型和工具调用树，适合检查实际输入输出、耗时和调用关系。
2. 现有 `context_compaction` custom event 记录不含正文的确定性阶段数据，适合脚本断言 L1→L2→L3→L5 顺序和各级前后变化。

只看 LangSmith 中是否出现摘要模型调用无法证明 L1/L2/L3 是否执行；只看压缩事件也无法分析模型为什么选择某个工具。当前部署已实测 LangGraph metadata 包含 `request_id` 和 `thread_id`；验收脚本同时输出这两个标识与 Agent `run_id`，前两者用于检索 LangSmith，后者用于查询 DocMind 运行记录和 SSE。不能假设当前 LangSmith metadata 一定包含 Agent `run_id`。

部署按需在受保护的 `.env` 设置以下变量，不得把 API key 写入仓库、文档或脚本：
...
SDK 0.8.7 会优先识别 `LANGSMITH_TRACING`，无需同时写入旧版 `LANGCHAIN_TRACING_V2`。环境变量由 Compose `env_file` 注入，因此修改后必须重建而不只是 restart `api`、`worker`。当前 `docmind-dev` 已验证变量解析、LangSmith 鉴权连通和服务健康；密钥只保存在远程 `.env`。

### 19.2 事件数据形状

只有初始请求超过 `prompt_budget` 或进入 provider overflow 恢复时才创建事件周期，避免正常短对话产生大量 skipped 噪声。同一 `cycle_id` 内固定顺序如下：

```text
L1 finished/skipped
  -> L2 finished/skipped
  -> L3 finished/skipped
  -> L5 started -> finished/failed
     或 L5 skipped（前三级已经满足预算）
```

公共字段包括 `level`、`sequence`、`cycle_id`、`reason`、`tokens_before/after/saved` 和 `messages_before/after`。L1～L3 增加 `candidate_messages`、`protected_messages`、`input_externalized`、`tool_results_projected`、`tool_arguments_projected`；L5 增加 `messages_removed`、`rounds_removed`、`archive_count/path`、`summary_revision/quality`。所有事件禁止携带用户正文、摘要正文、工具参数、工具结果或 Skill 正文。

### 19.3 场景和命令

`validate_context_compaction_api.py` 会通过真实 chat-iframe 身份创建不同会话、串行创建 Agent Run、消费 verbose SSE、校验每轮输入/输出/工具/线程文件/压缩事件并打印总结。`{payload}` 由 `--payload-chars` 动态生成，避免在仓库提交大测试正文。

在 32K `openai:qwen3.6:35b` 测试部署上分别运行：

```bash
cd backend
uv run python scripts/validate_context_compaction_api.py \
  --base-url http://192.168.1.220:5050 \
  --scenario-file scripts/scenarios/context_compaction_l1.json \
  --payload-chars 140000 --run-timeout 300

uv run python scripts/validate_context_compaction_api.py \
  --base-url http://192.168.1.220:5050 \
  --scenario-file scripts/scenarios/context_compaction_l2.json \
  --payload-chars 6000 --run-timeout 300

uv run python scripts/validate_context_compaction_api.py \
  --base-url http://192.168.1.220:5050 \
  --scenario-file scripts/scenarios/context_compaction_l3.json \
  --payload-chars 1000 --run-timeout 300

uv run python scripts/validate_context_compaction_api.py \
  --base-url http://192.168.1.220:5050 \
  --scenario-file scripts/scenarios/context_compaction_l5.json \
  --payload-chars 6000 --run-timeout 300
```

`context_compaction_l2.json` 使用历史工具结果验证 L2 结果投影；常见的大工具结果会先被文件系统中间件外置，不能依赖它们在压缩器前仍保留原始正文。由于模型回复和 Token 估算会让越过预算的具体轮次前后浮动，L2 在线程级断言任一轮确实完成投影，逐轮仍分别校验回复和工具行为。`context_compaction_l3.json` 自行生成四个早期大工具结果和十个近期小结果，`--payload-chars` 只需满足脚本的最小参数约束。`context_compaction_l5.json` 将压力分散到四个旧用户轮次，使最新请求仍可准入，并在线程级断言任一轮确实触发 L5；中间登记轮只确认唯一文件名，最终恢复轮仍严格断言合同号、禁止 L4、逐字完整路径和下一步待办，避免把本地模型在即时回复中偶发漏写路径前导斜杠误判为 checkpoint 丢失。

当前 32K 测试部署的真实证据如下；数值是保守 token 估算而非 provider 账单值：

| 层级 | thread_id | request_id / Agent run_id | 关键前后变化 | 恢复结果 |
| --- | --- | --- | --- | --- |
| L1 | `d97d8e86-2bd1-48a0-b423-4f20cae55a24` | `ctx-0803-184552-32c308-d0054c57be6b` / `15c2568c-1d3a-4f5f-b4a3-ff3489428881` | `146198 -> 13055`，外置当前输入 1 条 | 模型通过 `read_file` 恢复归档输入 |
| L2 | `0a4062be-0f66-4186-81c2-002613ddbee0` | `ctx-0803-184626-a58d41-c020c4bf1726` / `417ec1ea-04f5-47e7-9ac2-244f9d252951` | `31571 -> 26271`，投影历史工具参数 1 条 | 历史标记和线程文件保留 |
| L3 | `9739808b-bfe2-4bea-8bda-0f806ef31f26` | `ctx-0803-184732-5bad43-7f4b8737d235` / `df0e632d-39d9-4457-aee1-e39498b9bbdc` | `27790 -> 24869`，投影早期工具结果 1 条，保护近期消息 4 条 | 早期轮投影，最近两个闭合 round 保留 |
| L5 | `b26c3fe1-bc99-42e0-befc-1f5f2d1ec861` | `ctx-0803-180450-384704-e44ef0272658` / `c51091b2-9c49-4c9e-93a7-c6bfee6cf581` | `32648 -> 21239`，移除消息 3 条/round 1 个，归档 1 份 | 精确恢复禁止 L4、路径和下一步待办 |

正式通过必须同时满足：场景最终 `status=passed`；目标层存在 `finished` 且 `tokens_saved > 0`；同一周期的去重层级顺序严格为 L1、L2、L3、L5；输出保留场景标记/路径/约束；会话出现在 chat-iframe 对应业务 scope。单元测试另外验证同步/异步事件完全一致、真实 LangGraph custom stream 可见以及事件不含压缩前正文。
