...
- Integration 在缺少测试凭据的环境中执行：排除既有 `test_dataset_generation_resume_router.py` 的 `pg_manager` dispose fixture 挂起后，其余为 17 passed、138 skipped；跳过项需要 `TEST_USERNAME/TEST_PASSWORD` 或显式 provider connectivity 开关。该既有用例在凭据 fixture 能够 skip 之前即进入 autouse 数据库重建并持续阻塞，不属于本次上下文改动，本阶段不顺手修改。E2E 共收集 7 项，全部因缺少测试凭据 skip，不能记为通过。
...
- 发布加固仍有一个环境阻塞：远端未配置 `E2E_USERNAME/E2E_PASSWORD` 或 `TEST_USERNAME/TEST_PASSWORD`，不能把 HTTP E2E 记为通过。真实 64K 第二窗口已通过临时加载 27B 完成验收并恢复原环境；HTTP E2E 仍不得用跳过、伪造 token 或修改共享数据库账号替代。
...
- [ ] 使用专用测试凭据完成 integration/HTTP E2E。
