...
`context_compaction_l2.json` 使用历史 `write_file` 大参数验证 L2 参数投影；常见的大工具结果会先被文件系统中间件外置，不能依赖它们在压缩器前仍保留原始正文。`context_compaction_l3.json` 自行生成四个早期大工具结果和十个近期小结果，`--payload-chars` 只需满足脚本的最小参数约束。`context_compaction_l5.json` 将压力分散到四个旧用户轮次，使最新请求仍可准入，并在线程级断言任一轮确实触发 L5。
