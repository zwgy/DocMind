...
- 新增 `test_context_projection.py` 及中间件回归，覆盖串行/并行、重复/未知/未闭合/跨 round ID、确认保护、单轮 30 个 round 的候选边界，以及结果/参数归档。`test_context_projection.py` 与 `test_context_compaction_middleware.py` 共 39 项通过，Ruff 通过；远程 Docker 验收待 `docmind-dev` DNS 恢复后补做，不进入 P4。
