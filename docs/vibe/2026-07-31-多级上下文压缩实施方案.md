...
> 设计基线：Claude Code 2.1.88 可验证源码行为 + Yuxi 当前运行边界
...
研究基于 `ChinaSiro/claude-code-sourcemap` 的 commit：
...
- system + tools 本身超过 prompt budget 时仍尝试摘要。
...
- [ ] P0 冻结单轮多工具、并行工具、Skill schema、length、fixed overhead 故障样本。
- [ ] P1 修正校准键、诊断 hash、fixed overhead 和 length 分类。
- [ ] P1 增加 Docker 内 Token 校准脚本并保存两类真实部署报告。
- [ ] P2 重命名 `summary.py`、Middleware、工厂和测试，保持既有行为。
- [ ] P2 主 Agent/SubAgent 只注册一个 `ContextCompactionMiddleware`。
- [ ] P3 新增 `context_projection.py`，实现共享 API-round grouping、严格协议校验和安全载荷投影。
- [ ] P3 接入 L2 旧历史投影，确认不删除完整 AI/Tool round。
...
- [ChinaSiro/claude-code-sourcemap](https://github.com/ChinaSiro/claude-code-sourcemap/tree/main)
