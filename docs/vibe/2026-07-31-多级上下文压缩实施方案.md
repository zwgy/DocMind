...
只看 LangSmith 中是否出现摘要模型调用无法证明 L1/L2/L3 是否执行；只看压缩事件也无法分析模型为什么选择某个工具。两者通过 Agent Run metadata 中的 `run_id`、`request_id` 和 `thread_id` 关联。
