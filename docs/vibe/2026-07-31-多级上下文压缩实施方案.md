...
- 发布加固仍有两个环境阻塞：远端未配置 `E2E_USERNAME/E2E_PASSWORD` 或 `TEST_USERNAME/TEST_PASSWORD`，不能把 HTTP E2E 记为通过；模型缓存与 Ollama 当前也只有一个 32K 本地 Chat 部署，没有可用于 64K～256K 真实窗口验收的第二实例。两项均不得用跳过、虚假 profile 或未声明窗口的外部模型替代。
...
- [ ] 使用专用测试凭据完成 integration/HTTP E2E，并使用第二个实际 64K～256K 本地模型完成发布加固验收。
