...
| 初稿日期 | 2026-08-05 |
| 最后复核 | 2026-08-10 |
| 状态 | 阶段 0 至阶段 5 已完成代码实施与本地验证；真实 Docker Compose/PostgreSQL 端到端验收待环境补跑 |
...
- [x] Web 收件箱和运行历史展示状态、完成时间、摘要、产物数及“查看结果”，复用 `/agent/{thread_id}` 打开完整 Conversation；会话列表平铺显示 `MessageSquare / Clock3` 并保持选中置顶，定时会话隐藏删除且后端同步保护。
- [x] `chat-iframe` 接入同一结果卡片和定位流程，使定时会话跨 scope/宿主 Agent 可见；续聊和中断恢复使用 `currentThread.agent_id`，不回退宿主 Agent。
- [x] 完成本阶段本地静态与回归验证：后端相关单元测试 77 项、格式化和 Ruff 通过；chat-iframe 全量测试 177 项中 174 项通过、3 项既有跳过，类型检查、Lint 和生产构建通过；Web Lint 和生产构建通过。
- [ ] 当前宿主无 Docker CLI 且未配置 `POSTGRES_URL`，真实 PostgreSQL、Docker Compose 与浏览器 E2E 待可用环境补跑；该环境缺口不计为代码断言失败。
...
- [x] Web 与小助手会话列表通过一次列表请求平铺普通会话和定时执行会话，图标可区分；普通会话保持既有 scope 过滤，定时会话跨 scope/宿主 Agent 可见，当前选中会话重新打开侧栏仍置顶。
