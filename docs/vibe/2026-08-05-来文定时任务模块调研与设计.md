...
`agent` 动作只接受 `agent_slug`、`instruction` 和受控的 `timeout_seconds`。创建或编辑时，后端以当前登录用户复用 `AgentRepository` 的可见性检查，拒绝 SubAgent、不可见 Agent，以及由前端或模型提交的工具、知识库、所有者和运行配置字段。调度运行快照保存目标 Agent、指令、超时和创建时的 Agent 配置摘要，供审计与问题定位；实际执行仍通过既有 Agent Run 按目标 Agent 当时由管理员保存的配置装配工具、知识库、MCP 和 Skills。

个人自然语言创建继续复用既有 `create_personal_scheduled_task` 工具及其调用幂等键，而不是新增可绕过原有身份边界的平行工具。该工具的严格联合 Schema 现在同时接受通知和 Agent 动作；`scheduled-task` Skill 明确要求缺少目标 Agent、执行指令或超时时使用 `ask_user_question`，且绝不允许模型提交能力覆写字段。

这一区分是本轮对原方案“配置快照”表述的澄清，而非功能缩减：快照不能成为绕过管理员后续配置的运行时覆写入口。尤其是 `scheduled-task` Skill 与其他 Skill 一样，只在管理员将其配置到目标 Agent 后才生效，调度任务不注入、移除或替换任何 Skill。
