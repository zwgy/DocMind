...
- 在 Docker Compose 增加服务 `scheduler` 和 `dispatcher`，开发容器名分别为 `scheduler-dev` 和 `dispatcher-dev`；两者与后端共用镜像、源码挂载和 PostgreSQL，入口分别为 `uv run --no-sync --no-dev python -m yuxi.scheduled_jobs.scheduler` 和 `uv run --no-sync --no-dev python -m yuxi.scheduled_jobs.dispatcher`。
...
每次成功抽取都必须经过“构建来文任务批次”步骤，不能先把来文标记为 `ready`、再异步补建候选。处理顺序固定为：

1. 来文保持 `extracting`，创建或更新批次为 `building`。
2. 读取本次成功的 `extraction_run_id`；把旧运行候选标记为 `stale`。
3. 将本次抽取的所有任务明细转换为候选并执行统一领域校验。
4. 在同一数据库事务中保存全部候选、把批次改为 `ready`，最后把来文改为 `ready`。
...
- 草稿来文且不存在已启用任务时，可以物理删除；同一事务级联删除批次、候选和抽取结果。
...
- 来文外键删除策略为：批次对草稿来文 `ON DELETE CASCADE`，已启用任务通过候选唯一引用形成 `RESTRICT`，服务层在删除前给出可读错误。
...
- `resolved` 必须得到唯一 UID；多个候选为 `ambiguous`，无结果为 `not_found`。
...
| `owner_uid` | 原始来文上传账号；重新抽取不得改变所有者 |
| `name` / `notification_content` | 规范化草稿 |
...
新运行以 `attempt_count = 0`、`status = pending`、`next_attempt_at = database_now()` 创建。每次认领递增尝试次数，首次认领设置 `started_at`；终态必须设置 `finished_at`、清空租约和 `next_attempt_at`。数据库 `CHECK` 固定“只有 `dispatching` 可以保存完整租约、其他状态的租约字段必须为空”，认领服务在同一事务内使用 PostgreSQL `now()` 判断租约是否过期。调整原因：`CHECK` 中使用随时间变化的 `now()` 不能在时间流逝后自动重新校验既有行，无法可靠地表达“未过期”这一动态事实；该事实必须由认领查询和行锁保证。
...
- Compose 健康检查分别调用 `uv run --no-sync --no-dev python -m yuxi.scheduled_jobs.scheduler --health-check` 和 `uv run --no-sync --no-dev python -m yuxi.scheduled_jobs.dispatcher --health-check`，验证数据库连通且本实例心跳不早于 30 秒。
...
- [ ] 新增任务、接收人、运行表及 Repository/Service。
- [ ] 实现三类 Schedule Schema 和统一确定性校验。
- [ ] 实现通知 Action Handler、收件箱事件幂等、聚合已读和游标查询。
- [ ] 实现独立 Scheduler、Dispatcher、认领事务、租约、退避、有限补发和心跳。
- [ ] 在 Compose 增加 `scheduler`、`dispatcher` 服务、共享配置和健康检查。
- [ ] 完成时间矩阵、双实例并发、操作竞争、崩溃恢复和最大重试测试。

### 阶段 2：第一版来源接入

- [ ] 扩充来文任务抽取结构，实现 `building / ready / failed / frozen` 批次、零任务批次及失败重试。
- [ ] 实现本地 `PersonnelDirectory`、全体范围和人工修正。
- [ ] 实现候选人工“启用”和来文确认批量启用；无效候选保留原因且不阻塞来文确认。
