...
- [x] 小助手以单一时钟入口提供“定时中心”：默认“我的定时”展示当前认证用户的进行中、已暂停和历史通知/Agent 定义，并支持编辑、暂停、恢复和取消；“收件箱”只展示已发生的通知投递、通知异常和 Agent 执行状态，红点只统计后者的未读事件（2026-08-07：iframe 生产构建、类型检查和 ESLint 通过）。
