...
| 初稿日期 | 2026-08-05 |
| 最后复核 | 2026-08-10 |
| 状态 | 阶段 0 至阶段 4 的调度与 Agent 执行基础已完成；阶段 5“Agent 结果展示闭环”待实施 |
...
第二版 Agent 自动执行基础已经实现，但最终回复投影、统一会话发现、结果导航和绑定 Agent 续聊仍按阶段 5 实施。当前创建契约必须显式指定 `notification` 或 `agent`：前者只提醒，后者必须进入独立会话、Agent Run 和 ARQ 执行链路；不得因缺少动作判别字段而默认创建通知。
...
8. Agent Worker 在 Agent Run 状态提交后调用定时结果同步作为快路径。Dispatcher 每轮另以固定批量连接查询 `action_type = agent AND status IN (queued, running)` 且 Agent Run 当前状态与调度镜像不一致的记录，按调度运行 `updated_at, id` 从旧到新处理：状态已经变为 `running` 时补写 `running`，已经终态时调用与快路径相同的第 15.8 节结果投影事务。查询使用 `FOR UPDATE SKIP LOCKED` 和上述部分索引；仍在正常排队或执行且状态一致的长任务不进入对账批次。投影失败会使整笔镜像更新回滚，下一轮仍能命中；成功后状态一致或已终态，自然退出对账集合。这样 Worker 在两次提交之间退出或同步数据库暂时失败时仍能恢复，不另建第二套状态转换逻辑。
...
4. `artifact_count` 复用 `yuxi.agents.artifacts.delivered_artifact_paths` 从该线程 assistant 消息的结构化产物元数据提取路径，按现有虚拟路径规范化后去重计数；不扫描工作目录，不解析 assistant 文本猜测附件，也不把同一路径的重复事件重复计数。
...
- 当前选中会话置顶继续复用父分支既有逻辑。若重新打开左侧列表时当前线程不在已加载页，客户端先用单会话定位接口补入顶部，再执行同一选中置顶逻辑；这个显示层置顶不改变服务端 `limit + offset` 排序，也不写入新的业务状态。
...
若历史 Conversation 绑定的 Agent 后续被删除或当前用户失去可见权限，历史消息和仍有权限的产物保持只读可查，输入框禁用并明确提示“原 Agent 已不可用，无法继续对话”；后续定时运行进入失败并产生未读终态事件，不自动改用默认 Agent。这样可以保持执行语义和审计一致，避免同一任务在不知情的情况下换 Agent。
...
- `thread_kind=scheduled_run` 的 Conversation 在运行历史保留期间禁止通过通用会话删除接口删除，固定返回 `409 scheduled_run_conversation_protected`；取消任务不删除已经产生的结果会话。未来若引入数据保留期，必须由任务运行、收件事件、Conversation、消息和产物统一归档。
...
| 查看 Agent 定时运行 Conversation | 允许查看自己的任务运行 | 拒绝 | 拒绝 |
| 继续 Agent 定时运行 Conversation | 绑定 Agent 仍可见时允许 | 拒绝 | 拒绝 |
| 删除 Agent 定时运行 Conversation | 拒绝；随运行历史保留 | 拒绝 | 拒绝 |
| 修改自己的已读状态 | 允许 | 允许 | 拒绝 |
...
### 阶段 5：Agent 结果展示闭环（待实施）

- [ ] 按第 11.9 节顺序增加 `scheduled_job_runs.conversation_thread_id`、终态 `result_data` 固定字段、非空唯一约束和定时会话部分表达式索引；为既有终态 Agent 运行回填 `thread_id` 与结果投影，无法回填的历史记录保留原状态并记录迁移告警，不伪造结果入口或历史未读。
- [ ] 在 Agent 消息与产物提交后实现第 15.8 节幂等结果投影和第 13.4 节 Dispatcher 有界对账，保证终态收件事件晚于投影可见，并覆盖成功、失败、取消、无最终消息、重复回调、同步中断恢复和产物去重。
- [ ] 扩展运行历史和任务收件箱响应，增加按 run 已读；为会话列表增加 `include_scheduled_runs`、`thread_kind` 和单会话定位接口，并用真实 PostgreSQL 验证组合排序、`limit + offset` 分页、索引和所有权隔离。
- [ ] Web 收件箱和运行历史展示状态、完成时间、摘要、产物数及“查看结果”，复用 `/agent/{thread_id}` 打开完整 Conversation；会话列表平铺显示 `MessageSquare / Clock3` 并保持选中置顶，定时会话隐藏删除且后端同步保护。
- [ ] `chat-iframe` 接入同一结果卡片和定位流程，使定时会话跨 scope/宿主 Agent 可见；续聊改用 `currentThread.agent_id`，绑定 Agent 不可用时进入只读状态。
- [ ] 完成第 18 节新增后端、Web、chat-iframe 测试，执行格式化、Ruff、类型检查、Lint、生产构建，并在真实 Docker Compose 中验证一次性 Agent、周期 Agent 两次运行、通知隔离、跨 scope、逐 run 已读、结果定位和续聊。
- [ ] 同步 `ARCHITECTURE.md`、API 文档、用户说明和 `docs/develop-guides/changelog.md`，记录统一会话列表及结果事实来源。
...
- [ ] Agent 运行终态能稳定投影最终回复摘要和去重产物数量；运行历史、Web 收件箱和小助手收件箱展示一致，完整内容只从 Conversation 读取。
- [ ] Web 与小助手会话列表通过一次列表请求平铺普通会话和定时执行会话，图标可区分；普通会话保持既有 scope 过滤，定时会话跨 scope/宿主 Agent 可见，当前选中会话重新打开侧栏仍置顶。
- [ ] “查看结果”能定位当前分页外的 Conversation，成功打开后只标记对应 run 已读；失败不误清未读，周期任务多次运行互不影响。
- [ ] 定时执行结果续聊使用 Conversation 绑定 Agent；宿主 Agent 不同也能正常继续，原 Agent 不可用时历史只读且不自动切换默认 Agent。
- [ ] 一次性和周期 Agent 任务端到端执行、通知不创建会话、权限隔离、迁移回填、双端构建及真实 Docker Compose 验收全部通过，阶段 5 checklist 全部关闭。
