...
收件箱右侧的“清空已读”属于中性的列表整理操作，使用 `ListX` 等清理列表语义的图标和普通文字色，不使用红色删除图标；危险色只保留给单条“删除”。任务卡片底部统一用同一行、同一尺寸展示“查看结果 / 查看来源 / 删除”，空间不足时整体换行，不能把删除单独拆到第二列或使用明显不同的按钮高度。
...
- [x] 收件箱增加单条“删除”和分类“清空已读”；个人来源物理删除条目，来文来源设置当前接收人的 `hidden_at`，未来周期事件正常显示。
- [x] 个人定时结果 Conversation 允许通过会话入口独立删除；来文历史 Agent Conversation 保持保护。
- [x] Web 与小助手补齐确认交互、删除按钮、清空已读、加载状态与错误提示，并刷新列表和未读计数。
