...
| 初稿日期 | 2026-08-05 |
| 最后复核 | 2026-08-10 |
| 状态 | 阶段 0 至阶段 6 已完成代码实施、本地验证及真实 Docker Compose/PostgreSQL 浏览器端到端验收 |
...
- **历史**：已完成或已取消任务，按完成或取消时间降序，只读展示。个人通知任务的卡片只展示任务名称、状态、调度规则和最近触发时间，不重复展示已由“收件箱 → 通知”承载的通知正文；Agent 任务仍展示执行指令，通知正文也仍可在任务详情中查看。来文管理历史保留来源业务内容，避免管理员失去判断上下文。
...
收件箱右侧的“全部已读 / 删除已读”属于同一分类的批量操作，必须相邻组成紧凑操作组；“全部已读”使用 `CheckCheck` 和主色，“删除已读”使用 `Trash2` 和柔和错误色，空间不足时整组换行，避免用户混淆两个操作。任务卡片底部统一用同一行、同一尺寸展示“查看结果 / 查看来源 / 删除”，空间不足时整体换行，不能把删除单独拆到第二列或使用明显不同的按钮高度。
...
| Web / 小助手收件箱“任务” | 发现无人值守执行结果并处理未读 | 状态、完成时间、最终回复摘要、产物数量、“查看结果 / 查看来源” | `inbox_items` 加 `scheduled_job_runs` 结果投影、任务来源快照 |
...
- 从 Web 或小助手会话内创建个人任务时，在任务的不可变 `source_snapshot.thread_id` 中保存创建任务的会话，仅用于“查看来源”导航，不把原会话上下文带入无人值守执行。HTTP 创建和缺少该快照的历史任务不显示“查看来源”；原会话后续可独立删除，导航失败时明确提示“创建任务的会话已不存在”。结果会话不增加来源横幅，原会话也不增加任务工具卡片。
...
“查看来源”复用同一个单会话定位接口和权限校验：成功时 Web 跳转到原有 `/agent/{thread_id}`，小助手关闭定时中心并切换现有聊天区。该操作只是在创建会话与结果会话之间导航，不调用运行已读接口；只有“查看结果”成功打开后才清除对应运行的未读状态。
...
- 收件箱支持单条“删除”和按分类“删除已读”；“删除已读”严格排除未读记录，不删除或隐藏关联 Conversation。
...
- [x] 增加 `inbox_items.hidden_at` 和 `scheduled_job_user_states`，所有列表、游标、未读统计默认排除当前用户隐藏数据。
- [x] 个人 API 固定过滤 `source_type=personal`；个人删除在任务行锁内拒绝在途运行，并显式删除收件事件、运行、接收人、任务审计和任务定义。
- [x] 来文任务不再通过上传者 `owner_uid` 授权；新增管理员列表及暂停、恢复、取消、按当前管理员隐藏接口，上传仍保持 `get_required_user`。
- [x] 新建来文任务固定为 `notification`，在领域和数据库边界拒绝新的来文 Agent 任务。
- [x] 收件箱增加单条“删除”和分类“删除已读”；个人来源物理删除条目，来文来源设置当前接收人的 `hidden_at`，未来周期事件正常显示。
- [x] 个人定时结果 Conversation 允许通过会话入口独立删除；来文历史 Agent Conversation 保持保护。
- [x] Web 与小助手补齐确认交互、删除按钮、删除已读、加载状态与错误提示，并刷新列表和未读计数。
- [x] 完成 PostgreSQL 迁移、领域/权限/分页测试、双端静态检查、Compose 集成测试和真实浏览器端到端验收。
...
2026-08-11 在 `docmind-dev` 完成阶段 6 最终验收。真实 PostgreSQL 迁移已创建 `inbox_items.hidden_at` 与 `scheduled_job_user_states`，`scheduled_jobs.owner_uid` 已改为可空，既有来文通知的个人 owner 清零；迁移兼容实际 `conversations.extra_metadata` 的 `json` 类型并可重复执行。后端聚焦单元测试 38 项、PostgreSQL 集成测试 8 项全部通过，API、Scheduler、Dispatcher 均保持健康。小助手中删除个人 Agent 历史任务后任务卡消失，既有结果会话仍保留在历史对话首屏并显示独立“删除”入口；“清空已读”实际清除 8 条已读通知且保留 1 条未读，随后单条删除后列表为空。Web 管理端可在“我的定时 / 来文任务”之间切换，个人历史删除确认明确保留结果会话，来文任务使用管理员专属列表。双端浏览器控制台均无错误。
