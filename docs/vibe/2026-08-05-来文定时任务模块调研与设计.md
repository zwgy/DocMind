...
- [x] 优化小助手定时中心的信息层级：标题稳定居中，两组二级页签复用同一布局和视觉状态；通知正文或 Agent 指令直接进入任务卡片，常见 Cron 转换为中文周期描述，复杂表达式保留原值；历史记录区分最近触发和尚未触发（2026-08-07：chat-iframe 159 项测试、类型检查、ESLint、生产构建和真实 460px iframe 验证通过）。
