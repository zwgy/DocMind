...
- [x] `P0-5` run 终态历史校准与正式消息 ID — 状态：已完成
  - 实现：run 结束后重新拉取线程历史，按 `request_id` 仅替换本轮乐观消息；服务端尚未返回该消息段时保留本地展示。历史中的 `extra_metadata.response_metadata` 也会用于恢复模型名。
...
- [x] `P0-7` 闭环重试与反馈 — 状态：已完成
  - 实现：最后一条可操作回答显示“重新生成（追加新一轮）”；反馈只使用 P0-5 校准后的后端正式消息 ID，历史 `feedback` 会恢复已提交状态，点踩可填写原因；提交中的消息立即禁用，防止重复提交。
...
- [x] `P1-2` 首问自动生成会话标题 — 状态：已完成
  - 实现：复用 `/api/chat/call`，通过受限 `meta.use_fast_model` 标记由服务端选择 fast model；仅默认标题的首轮成功后异步更新，人工重命名或生成失败均不会影响聊天与标题。
...
- [x] `P1-6` 补齐高频消息操作 — 状态：已完成
  - 实现：用户消息增加复制成功反馈；图片支持全屏预览、遮罩/关闭按钮与 ESC 退出；附件改为显示图标、大小和上传中/已上传/失败状态，长文件名省略显示。助手操作区仍只渲染在最后一条完成回答下方。
...
- [ ] `P1-8` 建立核心体验测试门禁 — 状态：进行中
  - 实现：复用既有模型记忆/自动标题、分页/来源、上传校验和 Markdown 回归；新增消息复制、图片预览与附件状态的组件约束测试，并新增 `corepack pnpm test:p1` 一键执行全量测试、类型检查、lint 和生产构建。
  - 本地验证：`corepack pnpm test:p1` 已通过（86 通过、3 跳过），完成全量测试、类型检查、lint 和生产构建；构建仅保留既有 500 kB chunk 提示。
