...
| P2-3 文件与产物 | 纳入为“本轮交付物”卡片 | 最终回答下方显示该回答关联的 `present_artifacts` 文件，提供预览和下载。`agent_state.artifacts` 会在同一线程累积，不能直接当作本轮清单；也不提供通用线程文件树。 |
...
- [x] `P2-3` 呈现本轮交付物并支持下载 — 状态：已完成
  - 实现：以 `present_artifacts` 工具调用为用户交付物来源（该工具已限制为 `outputs` 下的文件），由后端写入最终助手消息的 `presented_artifacts` 元数据并随历史返回；前端只按该字段在最终回答下渲染卡片，不把它伪装成另一条消息，也不靠相邻工具调用猜测归属。下载和预览均经带 token 的 API 请求取得 Blob；图片、PDF、文本在 iframe 覆盖层预览，Office 文件明确显示“下载后查看”，不展示失效的预览按钮。不得调用通用 `/files` 浏览沙盒、workspace 或历史中未声明的文件。
  - 验证：新增后端持久化产物元数据单测通过；前端覆盖历史元数据归一化和带 token 的预览/下载入口；`corepack pnpm test:p1`（92 通过、3 跳过）通过。图片/PDF/文本可预览和下载，Office 仅下载；跨会话和无权路径均不能显示或读取。
  - 依赖：P0-5。
...
| P2-2 | 2026-07-15 | 当前工作区 | `corepack pnpm test:p1`：92 通过、3 跳过；类型检查、lint、生产构建通过 | 模型选择左侧按需展示最近一次模型调用的上下文用量与剩余量。 |
