...
| P2-2 Token、Todo、状态 | 纳入 Todo/状态与上下文按钮 | Todo 归入消息流运行卡片；输入区模型选择左侧增加“上下文”图标按钮，点击弹出小窗显示最新一次模型调用的已用/总量、剩余和占用进度。无 `token_usage` 时按钮不显示。 |
...
- [x] `P2-2` 展示 Todo 进度与按需上下文用量 — 状态：已完成
  - 实现：运行卡片从 `agent_state.todos` 计算完成数和当前项，最多展示后端已限制的 20 项；在输入区模型选择左侧增加仅有图标的“上下文用量”按钮，点击后显示最新模型调用的 `llm_input_tokens / context_window`、剩余量和进度条。这里展示的是最近一次模型调用快照，不伪装成精确的整段会话总 Token。
  - 验证：无 `token_usage` 或模型未提供 context window 时不显示按钮；占用量达到 80% 时给出颜色告警但不阻塞发送；`corepack pnpm test:p1`（92 通过、3 跳过）通过。
  - 依赖：P2-1。
...
| P2-2 | 2026-07-15 | 当前工作区 | `corepack pnpm test:p1`：94 通过、3 跳过；类型检查、lint、生产构建通过 | 模型选择左侧按需展示最近一次模型调用的上下文用量与剩余量；未配置模型上限时仍展示已用 token，并明确总量未知。 |
