...
name: knowledge-base
slug: knowledge-base
description: "检索用户明确指定的知识库或已入库资料；不用于按来文、收文或文号查询业务库，此类请求必须使用 incoming-document。"
---

# 知识库技能

当用户要求基于项目知识库、内部资料、上传入库文档或知识图谱相关内容回答问题时，使用此技能。用户按来文、收文、文号或来文标题查询业务库时，即使来文尚未进入知识库，也必须改用 `incoming-document`。
