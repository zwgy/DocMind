...
def remove_graphviz_doctype(output: Path) -> None:
    """移除 Graphviz 固定写入的 SVG DTD，保持交付物符合统一安全校验。"""
    content = output.read_text(encoding="utf-8")
    # Graphviz 的 SVG DTD 指向外部规范地址；产物虽来自受控本地进程，
    # 但交付层禁止任何 DTD，避免预览器解析时产生外部实体或网络访问歧义。
    sanitized = re.sub(r"<!DOCTYPE\s+svg\b[^>]*>\s*", "", content, count=1, flags=re.IGNORECASE)
...
if any(not isinstance(node.get("id"), str) or not node["id"].strip() or len(node["id"]) > 64 for node in nodes):
    raise ValueError("节点 ID 必须是非空短文本")
allowed_kinds = {"start", "process", "decision", "end"}
if any(
    node.get("kind") not in allowed_kinds
...
for node in nodes:
    dot.append(
        f'"{esc(node["id"])}" [label="{esc(node.get("label", ""))}", shape={shapes.get(node.get("kind"), "box")}];'
    )
for edge in edges:
    dot.append(f'"{esc(edge["source"])}" -> "{esc(edge["target"])}" [label="{esc(edge.get("label", ""))}"];')
dot.append("}")
subprocess.run(["dot", "-Tsvg", "-o", request["output"]], input="\n".join(dot), text=True, check=True, timeout=20)
remove_graphviz_doctype(Path(request["output"]))
