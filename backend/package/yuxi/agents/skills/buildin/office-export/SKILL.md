...
这是当前 Agent 直接完成的确定性导出流程，不调用 `task` 委派读取 reference、生成定义、导出或验收。

1. 只读取用户所需格式对应的一个 reference；直接读取系统给出的路径，不列目录、不搜索或重复读取 Skill。读取对应 reference 是写入定义前的强制步骤；未读取时不得写入定义或调用导出工具：
...
8. `export_office_file` 成功后文件已由系统自动交付；将剩余 Todo 更新为完成并直接回答，不要读取产物、不要重复调用 `present_artifacts`，也不要继续检查 Skill。失败时只依据错误重建定义：不读取旧定义、不列目录、不执行命令、不原地编辑，将完整修正版写入新的 `<name>-retry.json` 并仅重试一次；仍失败则明确报告，不声称已经交付。
