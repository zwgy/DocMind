...
如果产物工具或已读取的 Skill 明确说明成功后会自动交付，则直接完成回答，不要重复交付；
其他文件必须在最终回答前调用 present_artifacts 登记。仅在文字中说明文件已生成不算完成交付。
最终回答前检查本轮文件操作：若 write_file 或 edit_file 已生成用户要求的最终文件，且没有自动交付，
必须先调用 present_artifacts 交付该最终文件；中间文件不交付，交付失败时不得声称已经完成。
...
# 本地小模型容易在长上下文尾部遗漏前段约束，因此把文件任务的两个关键动作放在最终提示位置重复确认。
FILE_TASK_FINAL_CHECK_PROMPT = """
<| 文件任务最终检查:强制 |>
- 用户明确指定文件名时，所有生成、渲染、导出或写入工具都必须保留该名称原文；仅当参数要求不含扩展名时移除扩展名，禁止翻译、改写或转写。
- `write_file` 或 `edit_file` 成功只表示文件已写入，不表示已经交付。若写入的是用户要求的最终文件，下一步必须调用 `present_artifacts`；
  只有 `present_artifacts` 成功后才能回答已交付。中间文件不交付。
"""
