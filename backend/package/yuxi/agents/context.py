...
## intent
用户稳定的整体目标、任务范围和最终交付物。不要把某一轮临时回复格式或工具使用方式误判为整体目标。

## concepts
后续工作仍需使用的技术概念、架构边界、长期约束、已确认决策和设计原则；不要记录消息流水。
...
## progress
已有可靠结果证明完成的步骤、工具核验结果、关键结论、已确认的方案和决策，以及被否定的方案及原因。

## user messages
按先后顺序记录仍会影响任务的用户请求、纠正、硬约束、偏好、禁忌、技术取舍和验收标准，并明确取消、替换或更正关系。保留会影响任务理解的精确事实和原始标识，不要复制冗长工具输出。

## pending tasks
尚未完成、被阻塞或等待验收的任务。已经完成的单轮回复格式或工具使用要求不得列为待办。

## current work
压缩发生时尚未完成的具体工作、所处阶段、中断位置和最近一次可靠状态。

## next step
紧接着应执行的一个或少量明确动作，以及继续前必须读取或核验的对象。已经完成的旧轮临时输出格式或工具使用要求不得列为下一步，
除非用户明确要求后续继续执行；没有下一步时写 None。
...
class BaseContext:
    ...
    summary_prompt: str = field(
        ...
        metadata={
            "name": "上下文摘要提示词",
            "description": (
                "定义摘要提取内容和输出结构，必须包含 {messages} 占位符。"
                "框架会另外追加累计合并、输入隔离、精确值保护和输出预算协议。"
            ),
            "type": "string",
            "kind": "prompt",
            "auth": "admin",
