...
test('stream marks a synthetic tool error as failed without waiting for history reload', async () => {
  setActivePinia(createPinia())
  globalThis.fetch = async (url, options = {}) => {
    if (url === '/api/agent/runs' && options.method === 'POST') return Response.json({ id: 'run-1' })
    if (url === '/api/agent/runs/run-1') return Response.json({ run: { status: 'running' } })
    if (url.startsWith('/api/agent/runs/run-1/events')) {
      const body =
        sseBlock('1-0', 'messages', {
          payload: {
            items: [
              { status: 'running', stream_event: { type: 'tool_call', tool_call_id: 'call-list', name: 'list', args: {} } },
              {
                status: 'running',
                stream_event: {
                  type: 'tool_result',
  ...
  const toolCall = chat.messages.flatMap((message) => message.toolCalls || []).find((tool) => tool.id === 'call-list')
  assert.equal(toolCall?.status, 'error')
  assert.equal(toolCall?.result, 'list is not a valid tool')
})
