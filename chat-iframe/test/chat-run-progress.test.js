...
const tokenUsageSource = readFileSync(new URL('../src/utils/token-usage.ts', import.meta.url), 'utf8')
...
test('context usage is an optional input control next to model selection', () => {
  ...
  assert.match(inputSource, /上次模型输入/)
  assert.match(inputSource, /安全输入预算/)
  assert.match(inputSource, /模型协议\/模板校正/)
  ...
  assert.match(tokenUsageSource, /provider_usage: '模型服务实测'/)
  assert.match(tokenUsageSource, /calibrated_estimate: 'usage 校准估算'/)
