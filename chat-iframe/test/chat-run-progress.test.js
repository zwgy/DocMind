...
test('context usage is an optional input control next to model selection', () => {
  ...
  assert.match(inputSource, /prompt_budget/)
  assert.match(inputSource, /const limit = promptBudget \|\| Math\.max\(used, 1\)/)
  assert.match(inputSource, /可用输入预算剩余/)
  assert.match(inputSource, /模型完整上下文/)
  assert.doesNotMatch(inputSource, /summary_trigger_tokens/)
