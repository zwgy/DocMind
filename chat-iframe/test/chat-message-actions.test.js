...
const refsSource = readFileSync(
  new URL('../src/components/MessageRefs.vue', import.meta.url),
  'utf8'
)
...
test('user message actions keep copy, image preview and attachment state visible', () => {
  ...
  assert.match(source, /role="dialog"[\s\S]*?aria-modal="true"[\s\S]*?aria-label="图片预览"/)
...
test('user copy action stays reachable outside the bubble and appears on hover or keyboard focus', () => {
  assert.match(
    source,
    /class="user-message-copy"[\s\S]*?<\/button>\s*<div class="message-content">/
  )
  assert.match(
    styles,
    /\.chat-message\.user:hover \.user-message-copy,\s*\.user-message-copy:focus-visible/
  )
