...
function resetEditForm(job) {
  ...
  editForm.value = {
    ...
    runAt: toZonedDateTimeInput(job.run_at, job.timezone || 'Asia/Shanghai'),
    intervalValue: interval.value,
    intervalUnit: interval.unit,
    anchorAt: toZonedDateTimeInput(job.anchor_at, job.timezone || 'Asia/Shanghai'),
...
function scheduleSummary(job) {
  if (job.schedule_kind === 'at') return `单次 · ${formatTime(job.run_at, job.timezone)}`
...
function actionContent(job) {
  const value =
    job.action_type === 'agent' ? job.action_data?.instruction : job.action_data?.content
...
async function previewSchedule() {
  ...
  preview.value = null
  try {
    preview.value = await scheduledJobApi.preview({
  ...
  <div class="scheduled-jobs-view">
    ...
    <main class="scheduled-jobs-content">
      ...
      <a-alert
        ...
        type="error"
        show-icon
        :message="page.error.message || '加载任务失败'"
      >
        ...
        <article v-for="job in page.items" :key="job.id" class="job-row">
          <div class="job-main">
            <div class="job-title">
              ...
              ><a-tag :class="`status-${job.status}`">{{ statusText(job.status) }}</a-tag>
            </div>
            <div class="job-meta">
              <span>{{ job.source_type === 'incoming' ? '来文任务' : '个人任务' }}</span
          ...
          <div class="job-actions">
            ...
            <a-button
              ...
              @click="handleAction(job, 'pause')"
              ><Pause :size="16" />暂停</a-button
            >
            <a-button
              v-if="job.status === 'paused'"
              type="text"
              :loading="page.loading"
              @click="handleAction(job, 'resume')"
              ><Play :size="16" />恢复</a-button
            >
            <a-popconfirm
              v-if="['active', 'paused'].includes(job.status)"
              title="确认取消此任务？"
      ...
      <div v-if="page.cursor" class="load-more">
        <a-button :loading="page.loadingMore" @click="store.load()">加载更多</a-button>
      </div>
    </main>
    ...
    <a-drawer v-model:open="editorOpen" width="min(680px, 92vw)" title="编辑个人定时任务">
      <a-form layout="vertical">
        ...
        <a-form-item label="动作类型" required
          ><a-radio-group v-model:value="editForm.actionType"
            ><a-radio value="notification">站内通知</a-radio
            ><a-radio value="agent">执行 Agent</a-radio></a-radio-group
          ></a-form-item
        >
        <template v-if="editForm.actionType === 'notification'"
          ><a-form-item label="通知标题" required
            ><a-input v-model:value="editForm.title" :maxlength="100" /></a-form-item
          ><a-form-item label="通知正文" required
            ><a-textarea
              v-model:value="editForm.content"
              :rows="4"
              :maxlength="4000" /></a-form-item
        ></template>
        <template v-else
          ><a-form-item label="目标 Agent" required
            ><a-select v-model:value="editForm.agentSlug" placeholder="选择可执行 Agent"
          ...
          ><a-form-item label="执行指令" required
            ><a-textarea
              v-model:value="editForm.instruction"
              :rows="5"
              :maxlength="8000" /></a-form-item
          ><a-form-item label="超时（秒）" required
            ><a-input-number
              v-model:value="editForm.timeoutSeconds"
              :min="60"
              :max="3600"
        ...
        <div class="form-grid">
          ...
          <a-form-item label="调度类型" required
            ><a-select v-model:value="editForm.scheduleKind"
              ><a-select-option value="at">单次</a-select-option
              ><a-select-option value="interval">间隔</a-select-option
              ><a-select-option value="cron">Cron</a-select-option></a-select
            ></a-form-item
          >
        </div>
        <a-form-item v-if="editForm.scheduleKind === 'at'" label="触发时间" required
          ><a-input v-model:value="editForm.runAt" type="datetime-local"
        /></a-form-item>
        <template v-else-if="editForm.scheduleKind === 'interval'"
          ><div class="form-grid">
            <a-form-item label="间隔" required
              ><a-space-compact class="full-width"
                ><a-input-number
                  v-model:value="editForm.intervalValue"
                  :min="1"
                  :precision="0"
                  class="interval-value"
                /><a-select v-model:value="editForm.intervalUnit" class="interval-unit"
                  ><a-select-option value="minutes">分钟</a-select-option
                  ><a-select-option value="hours">小时</a-select-option
                  ><a-select-option value="days">天</a-select-option></a-select
                ></a-space-compact
              ></a-form-item
            ><a-form-item label="首次触发时间" required
              ><a-input v-model:value="editForm.anchorAt" type="datetime-local"
            /></a-form-item></div
        ></template>
        <a-form-item v-else label="Cron 表达式" required
          ><a-input v-model:value="editForm.cronExpression" placeholder="0 9 * * 1-5"
        /></a-form-item>
        <a-alert
          v-if="preview"
          type="info"
          ...
          :message="`下一次：${formatTime(preview.next_run_at, editForm.timezone)}`"
          :description="preview.occurrences?.map((entry) => entry.local).join('；')"
        />
        <p class="edit-hint">任务一旦已产生运行记录将不能编辑，接收人始终为当前用户。</p>
        <div class="editor-actions">
          <a-button :loading="previewLoading" @click="previewSchedule">预览触发时间</a-button
          ><a-button type="primary" :loading="saving" @click="saveEditor">保存</a-button>
        </div>
      </a-form>
    </a-drawer>
  </div>
